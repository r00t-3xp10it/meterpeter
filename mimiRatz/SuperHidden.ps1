   ...
   ElseIf($Directory -ne "false")
   {
      ...
      If($FolderName -ne "false")
      {
          If($Recursive -ieq "true")
         {
            ...
            $SHdb = Get-ChildItem -Path "$Directory" -Recurse -Force| Select-Object * |
               ...
               $_.Attributes -Match '(Hidden|System)' -and $_.FullName -iNotMatch 'Packages'
            } | Select-Object FullName,CreationTime,LastAccessTime,Attributes | Format-Table -AutoSize
         }
         Else
         {
            ...
            $SHdb = Get-ChildItem -Path "$Directory" -Force| Select-Object * |
               Where-Object { $_.PSIsContainer -eq 'True' -and $_.Name -iMatch "$FolderName" -and $_.Attributes -Match '(Hidden|System)'
            } | Select-Object FullName,CreationTime,LastAccessTime,Attributes | Format-Table -AutoSize         
         }
      ...
      Else
      {
         ...
         If($Recursive -ieq "true")
         {
            ...
            $SHdb = Get-ChildItem -Path "$Directory" -Recurse -Force | Select-Object * |
               Where-Object { $_.PSIsContainer -eq 'True' -and $_.Attributes -iMatch 'Hidden, System' -and $_.FullName -iNotMatch 'Packages'
            } | Select-Object FullName,CreationTime,LastAccessTime,Attributes | Format-Table -AutoSize
         }
         Else
         {
