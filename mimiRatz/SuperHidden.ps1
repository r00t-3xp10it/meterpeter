...
.NOTES
   ...
   Remark: Admin privileges required to create folders outside userprofile tree.
...
$supertable.Columns.Add("Super Hidden Directorys")|Out-Null
...
If($Action -ieq "Query")
{
   ...
   .OUTPUTS
      Attributes      Super Hidden Directorys                 LastAccessTime                         
      --------------- -----------------------                 --------------
      Hidden, System  C:\Program Files\Windows Sidebar        11/18/2021 09:32:55 11/18/2021 09:32:55
      Hidden, System  C:\Program Files (x86)\Windows Sidebar  11/18/2021 03:13:06 11/18/2021 03:13:06
   #>
      ...
      ForEach($Item in $CommonLocations)
      {
         ...
         If($RawList)
         {
            ForEach($Entry in $RawList)
            {
               ...
               $LastAccessTime = (Get-ChildItem -Path "$Entry").LastAccessTime
               $supertable.Rows.Add("$Attributes","$Entry","$LastAccessTime")|Out-Null
            }
         }
      }
