...
.EXAMPLE
   ...
   Search for hidden,system folder on %TMP% location with the name 'vault'  

.EXAMPLE
   PS C:\> .\SuperHidden.ps1 -Action Hidden -Directory $Env:TMP -FolderName vault
   Create\modify hidden,system folder on %TMP% location with the name 'vault'   

.EXAMPLE
   PS C:\> .\SuperHidden.ps1 -Action Visible -Directory $Env:TMP -FolderName vault
   Create\modify VISIBLE,system folder on %TMP% location with the name 'vault'    

.EXAMPLE
   PS C:\> .\SuperHidden.ps1 -Action Delete -Directory $Env:TMP -FolderName vault
   Delete the super hidden folder on %TMP% location with the name 'vault'    
