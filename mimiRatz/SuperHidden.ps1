...
If($Action -ieq "Query")
{
   ...
   .NOTES
      The -Recursive 'true' argument requires of an -Directory input by user.
   #>
   ...
   If($Directory -ieq "false")
   {
      ...
      ForEach($Item in $CommonLocations)
      {
         ...
         } | Select-Object FullName,CreationTime,LastAccessTime,@{Name='Attributes';Expression={'Hidden, System'}} | Format-Table -AutoSize
      }
   ...
   ElseIf($Directory -ne "false")
   {
      ...
      If(-not(Test-Path -Path "$Directory" -EA SilentlyContinue))
      {
         ...
         Write-Host "error: directory not found: '$Directory'`n" -ForegroundColor Red -BackgroundColor Black
...
If($Action -ieq "Delete")
{
   ...
   If(-not(Test-Path -Path "$Directory\$FolderName" -EA SilentlyContinue))
   {
      ...
      Write-Host "Error: directory '$Directory\$FolderName' not found.`n" -ForegroundColor Red -BackgroundColor Black
      exit #Exit SuperHidden
   }
