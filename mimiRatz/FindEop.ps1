<#
.SYNOPSIS
   Search for Escalation Of privileges [local]

   Author: @r00t-3xp10it
   Tested Under: Windows 10 (19043) x64 bits
   Required Dependencies: none
   Optional Dependencies: none
   PS cmdlet Dev version: v1.3.21

.DESCRIPTION
   Cmdlet to search for Escalation Of privileges [local].
...
$host.UI.RawUI.WindowTitle = "@FindEop v1.3.24 {SSA RedTeam @2022}"
...
If(Test-Path -Path "$Env:LOCALAPPDATA\Microsoft\Remote Desktop Connection Manager\RDCMan.settings" -ErrorAction SilentlyContinue)
{
   ...
   Write-Host "Directory    : $Env:LOCALAPPDATA\Microsoft\Remote Desktop Connection Manager" -ForeGroundColor Green
   Write-Host "vulnerablity : Credentials are stored inside [ .rdg ] files .." -ForeGroundColor Yellow
}
Else
{
   Write-Host "[RDP] not found: $Env:LOCALAPPDATA\Microsoft\Remote Desktop Connection Manager\RDCMan.settings"
}
Write-Host "`n"
...
If($verb -ieq "True")
{
   ...
   If($TESTFILES)
   {
      Write-Host "$TESTFILES" -ForeGroundColor Green -BackGroundColor Black
   }
   Else
   {
...
If(Test-Path -Path "$Env:USERPROFILE\Desktop" -EA SilentlyContinue)
{
   ...
   {
      Get-Content -Path "$FoundFile" -EA SilentlyContinue|Where-Object {$_ -iMatch '(Username|User:|user name|pass|Password|passwd)'} > $Env:TMP\cleartext.txt
   }

   If(Test-Path -Path "$Env:TMP\cleartext.txt" -ErrorAction SilentlyContinue)
   {
      Get-Content "$Env:TMP\cleartext.txt" -Encoding UTF8|Out-String -Stream|ForEach-Object {
         $stringformat = If($_ -iMatch '(Username|User|user name|pass|Password|passwd)')
         {
            @{ 'ForegroundColor' = 'Green' }
         }
         Else
...
If(Test-Path -Path "$Env:USERPROFILE\Documents" -EA SilentlyContinue)
{
   ...
   $FilesToScan = (Get-ChildItem "$Env:USERPROFILE\Documents" -EA SilentlyContinue).FullName|Where-Object {$_ -iMatch '(.log|.txt|.xml)$'}
   ForEach($FoundFile in $FilesToScan)
   {
      Get-Content -Path "$FoundFile" -EA SilentlyContinue|Where-Object {$_ -iMatch '(Username|User:|user name|pass|Password|passwd)'} > $Env:TMP\cleartext.txt
   }
...
Remove-Item -Path "$Env:TMP\cleartext.txt" -Force -ErrorAction SilentlyContinue
#return to pwd
cd $LocalPath
Write-Host "`n"
...
If(-not($bool))
{
   Write-Host "[DUMP] Administrator privileges required to dump." -ForeGroundColor Red -BackGroundColor Black
}
Else
{
