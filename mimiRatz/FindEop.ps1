...
If($Verb -ieq "True")
{
   #What processes loaded amsi.dll?
   Write-Host "lIST OF PROCESSES THAT LOAD AMSI.DLL"
   Write-Host "------------------------------------"
   ps | Where-Object { $_.Modules.ModuleName -contains 'amsi.dll' }
   Write-Host "`n"
}
