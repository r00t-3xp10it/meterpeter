...
If((Get-Service -Name "termservice" -EA SilentlyContinue).Status -ieq "Running")
{
   ...
   If($QueryTasts -iMatch 'rdpcorets.dll')
   {
      $Parse = $QueryTasts.Split('')|Where-Object {$_ -ne ''}
      #Grab PID from string: "svchost.exe 11656 rdpcorets.dll"
      $PPID = $Parse[1] #NOTE: PID cames after svchost name[1]
