...
IF NOT %1%.==. (
   ...
   echo ----------------------------------
   powershell -C "$TESTFILES = (Get-ChildItem -Path $Env:USERPROFILE -Recurse -Include 'credentials.db','access_tokens.db','accessTokens.json','azureProfile.json','legacy_credentials','gcloud' -Exclude 'Games','music','pictures','starcraft' -Force -EA SilentlyContinue).FullName;If($TESTFILES){Write-Host $TESTFILES -ForeGroundColor Green -BackGroundColor Black}Else{Write-Host '[CLOUD] not found: credentials in db files.' -ForeGroundColor Red -BackGroundColor Black}"
   echo.
   echo.
)
