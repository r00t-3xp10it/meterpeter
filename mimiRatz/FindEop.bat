...
del /f ttt.log  >nul 2>&1
del /f final.log  >nul 2>&1
for %%A in (%Path%) do (echo %%A >> ttt.log)
FOR /F "tokens=* USEBACKQ" %%F IN (`type ttt.log`) DO (cmd.exe /c icacls "%%~F" 2>nul | findstr /i "(F) (M) (W)" | findstr /i ":\\ everyone users todos %username%" | findstr /V "TrustedInstaller" >> final.log)
type final.log
del /f /q ttt.log
del /f /q final.log
echo.
