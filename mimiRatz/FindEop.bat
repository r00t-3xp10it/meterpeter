...
FOR /F "tokens=* USEBACKQ" %%F IN (`type EnvironementPaths.log`) DO (cmd.exe /c icacls "%%~F" 2>nul | findstr /i "(F) (M) (W)" | findstr /i ":\\ everyone users todos %username%" | findstr /V "TrustedInstaller" >> VulnDataBase.log)
