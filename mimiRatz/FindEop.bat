...
:: ----
TITLE FindEop v1.3.15 {SSA RedTeam @2022}


:: Local variables
SET LocalPath=%cd%
SET BatVersion=v1.3.15
...
powershell -C "Write-Host 'vulnerable if shell is running with low privileges' -ForeGroundColor Yellow"
echo --------------------------------------------------
powershell -C "$juicy = whoami /priv|findstr /i /C:'SeImpersonatePrivileges'|findstr /i /C:'Enabled';If(-not($juicy)){write-host 'ERROR: None vulnerable token privileges found.'}Else{echo $juicy}"
echo.
echo.
...
powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host 'Pipes Manipulation => [VULNERABLE::T1574]' -ForeGroundColor Green -BackGroundColor Black;$Report = $CheckPipes|Select -Skip 1|Select -First 5;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
echo.
echo.
...
echo ----------------------
powershell -C "$DomainControler = $Env:USERDOMAIN;Write-Host ::DCName::[$DomainControler]:[MITRE::T1069] -ForeGroundColor Yellow;nltest /DCNAME:$DomainControler|Out-Null;nltest /DSGETDC:$DomainControler|Out-Null;nltest /DCLIST:$DomainControler|Out-Null"
echo.
echo.
