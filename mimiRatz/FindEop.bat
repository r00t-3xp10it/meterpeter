...
powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host 'Named Pipes Token Manipulation => [VULNERABLE::TA0004]' -ForeGroundColor Green -BackGroundColor Black;$Report = $CheckPipes|Select -Skip 1|Select -First 5;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
echo.
echo.
...
echo ----------------
powershell -C "klist sessions|?{$_ -ne ''}"
echo.
echo.
...
powershell -C "Write-Host 'echo Use Mimikatz 'dpapi::cred' module with /masterkey to decrypt' -ForeGroundColor Green"
