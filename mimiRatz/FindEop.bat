...
powershell -C "$CheckPipes = (Get-ChildItem \\.\pipe\ -EA SilentlyContinue).FullName;If($CheckPipes){Write-Host 'Named Pipes Token Manipulation => [VULNERABLE::TA0004]';$Report = $CheckPipes|Select -Skip 1|Select -First 5;echo $Report}Else{Write-Host 'ERROR: None Name Pipes found ..'}"
echo.
echo.
...
%cacls_exe% "C:\Users\*" 2>nul | findstr "(F) (M) (W)" | findstr "%viriato% %token% %username%" 
%cacls_exe% "C:\Program Files\*" 2>nul | findstr "(F) (M) (W)" | findstr "%viriato% %token% %username%"
%cacls_exe% "C:\Program Files (x86)\*" 2>nul | findstr "(F) (M) (W)" | findstr "%viriato% %token% %username%"
%cacls_exe% "C:\Documents and Settings\*" 2>nul | findstr "(F) (M) (W)" | findstr "%viriato% %token% %username%"
echo.
echo.
