...
$CmdletVersion = "v2.4.9" #CmdLet version
...
ElseIf($LanguageSetting -iMatch '^(de-DE)$')
{
      $UserGroup = "Alle"                                      #Default scan
      $UtilGroup = "BUILTIN\\Benutzer"                         #Default scan
      $GroupFdx = "$Env:USERDOMAIN\\$Env:USERNAME"             #Default scan     - Only available with -action 'path'
      $OneMorek = "NT AUTHORITY\\Authentifizierte Benutzer"    #extra Group Name - Only available with -extragroup 'true'
   ...
   ForEach($TokenPath in $EnvironementPaths)
   {
      ...
      ForEach($ACLPermission in $DirectoryPermission)
      {
         If($Verb -ieq "True")
         {
            ...
            write-host "[$NewCounter] $FinalGroupList" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
   ...
   ForEach($Token in $ParseData)
   {
      ...
      ForEach($GroupServiceName in $FinalGroupList)
      {
         ...
         If($Verb -ieq "True")
         {
            ...
            write-host "[VERBOSE] Identity: " -ForegroundColor Blue -BackgroundColor Black -NoNewLine;
            write-host "[$NewCounter] $GroupServiceName" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
            Write-Host " - Permission " -ForegroundColor Blue -BackgroundColor Black -NoNewLine;
            Write-Host "$WeakPerm" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
            Write-Host "." -ForegroundColor Blue -BackgroundColor Black;
            Start-Sleep -Milliseconds 100
         }
