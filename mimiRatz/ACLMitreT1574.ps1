...
.NOTES
   ...
   If invoked -addextra 'true' @argument then cmdlet adds extra
   permissions to the 'ACL_Permissions_List' (Write,ReadAndExecute)
   Remark: addextra parameter takes a long time to finish if invoked
...
.EXAMPLE
   PS C:\> .\ACLMitreT1574.ps1 -action dir -addextra true
   Scan recursive in pre-defined directorys for pre-defined
...
If($Action -ieq "path")
{
   ...
   If($addextra -ieq "True")
   {
      #If -addextra 'true' add extra permissions
