...
If($Action -ieq "reg")
{
   ...
   If($Count -eq 0 -or $Count -ieq $null)
   {
      ...
      If($parseData -gt 1)
      {
         ...
         Write-Host "[ACL] Group: '$OneMorek'" -ForegroundColor Yellow
         Write-Host "[ACL] Group: '$UtilGroup'" -ForegroundColor Yellow
         Write-Host "[ACL] Group: '$UserGroup'" -ForegroundColor Yellow
      }
      Else
      {
