...
If($logfile -ieq "True")
{
   ...
   echo "Logfile created by ACLMitre1574" > $Env:TMP\ACLMitre1574.log
   echo "Scan Start: $ScanStartTimer" >> $Env:TMP\ACLMitre1574.log
   echo "------------------------------------------" >> $Env:TMP\ACLMitre1574.log
}
...
If($Action -ieq "path")
{
         ...
         (Get-Acl "$TokenPath").Access | Where-Object {#Search for Everyone:(F) \ Everyone:(M) directory permissions (default)
            ...
            If($CleanOutput)
            {
               ...
               If($logfile -ieq "True")
               {
                  ...
                  echo "VulnId            : ${Count}::ACL (Mitre T1574)" >> $Env:TMP\ACLMitre1574.log
                  echo "FolderPath        : $TokenPath" >> $Env:TMP\ACLMitre1574.log
                  echo "FileSystemRights  : $ACLPermission" >> $Env:TMP\ACLMitre1574.log
                  echo "IdentityReference : $MyReference" >> $Env:TMP\ACLMitre1574.log
                  echo "IsInherited       : $IsInHerit`n" >> $Env:TMP\ACLMitre1574.log
               }
   ...
   If($Count -eq 0)
   {
      Write-Host "[NONE] permissions found that match the search criteria."
      Write-Host ""
   }
...
If($Action -ieq "dir")
{
   ...
   If($scan -ne "false")
   {
      ...
      {
         Write-Host "* ERROR: not found '$scan'" -ForegroundColor Red -BackgroundColor Black
   ...
   ForEach($Token in $ParseData)
   {
      ...
      ForEach($GroupServiceName in $FinalGroupList)
      {
         ...
         If($CleanOutput)
         {
            ...
            If($logfile -ieq "True")
            {
               echo "VulnId            : ${Count}::SRV" >> $Env:TMP\ACLMitre1574.log
               echo "RegistryPath      : $Token" >> $Env:TMP\ACLMitre1574.log
               echo "IdentityReference : $GroupServiceName" >> $Env:TMP\ACLMitre1574.log
               echo "RegistryRights    : $WeakPerm" >> $Env:TMP\ACLMitre1574.log
               echo "AccessControlType : Allow" >> $Env:TMP\ACLMitre1574.log
               echo "IsInherited       : $IsInHerit`n" >> $Env:TMP\ACLMitre1574.log
            }
...
If($logfile -ieq "True")
{
   ...
   Write-Host "'$Env:TMP\ACLMitre1574.log'" -ForegroundColor Green -BackgroundColor Black;
}
