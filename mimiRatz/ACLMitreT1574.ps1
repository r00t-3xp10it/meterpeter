...
.NOTES
   ...
   Name to the 'Group_Names_to_scan_List' (Domain\Authenticated Users)
...
.EXAMPLE
   ...
   'FullControl, Modify' with 'Everyone,Users,UserName'
...
.EXAMPLE
   ...
   Scan recursive in pre-defined directorys + -extrapath 'string' for pre-defined
   UserGroups 'Everyone,Users' with 'FullControl,Modify' ACL permissions settings.
...
.OUTPUTS
   ...
   IdentityReference : SKYNET\Users
   IsInherited       : True
...
If($LanguageSetting -iMatch '^(pt-PT)$')
{
      ...
      $UtilGroup = "$Env:USERDOMAIN\\Utilizadores"             #Default scan
      $GroupFdx = "$Env:USERDOMAIN\\$Env:USERNAME"             #Default scan - only in -action 'path'
      $OneMorek = "$Env:USERDOMAIN\\Utilizadores Autenticados" #extra Group Name
}
Else
{
      $UserGroup = "Everyone"                                   #Default scan
      $UtilGroup = "$Env:USERDOMAIN\\Users"                     #Default scan
      $GroupFdx = "$Env:USERDOMAIN\\$Env:USERNAME"              #Default scan - only in -action 'path'
      $OneMorek = "$Env:USERDOMAIN\\Authenticated Users"        #extra Group Name
}
...
If($Action -ieq "path")
{
   ...
   {
      #If -extraperm 'true' add extra permissions
...
If($Action -ieq "dir")
{
   ...
   If($extrapath -ne "false")
   {
      ...
      If(-not(Test-Path -Path "$extrapath" -EA SilentlyContinue))
      {
         ...
         #Inputed directory path NOT FOUND [ -extrapath 'string' ] .. Use Default search then ..
