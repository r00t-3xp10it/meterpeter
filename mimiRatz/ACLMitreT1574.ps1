...
.NOTES
   ...
   Remark: Parameter -extrapath 'string' only works if invoked together
   with -action 'dir' @argument ( Scan recursive pre-defined paths ) and
   it will add the extra path to the end of 'Directorys_To_Scan_List' db.
...
.Parameter extrapath
   Add extra path to directorys_to_scan_list? (default: false)
...
.EXAMPLE
   PS C:\> .\ACLMitreT1574.ps1 -action dir -extrapath "C:\Users\pedro\Coding"
   Scan recursive in pre-defined directorys + -extrapath 'string' for 'Everyone,
   BUILTIN\Users' GroupNames with 'FullControl,Modify' ACL permissions settings.
...
If($Action -ieq "path")
{
   ...
   .NOTES
     If invoked -verb 'true' @argument then cmdlet displays in
     all the directory paths beeing scanned in realtime (slow)
   ...
   ForEach($TokenPath in $EnvironementPaths)
   {
      ...
      ForEach($ACLPermission in $DirectoryPermission)
      {
         If($Verb -ieq "True")
         {
            ...
            write-host "$FinalGroupList" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
            Write-Host " - Permission " -ForegroundColor Blue -BackgroundColor Black -NoNewLine;
            Write-Host "$ACLPermission" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
...
If($Action -ieq "dir")
{
   ...
   .NOTES
     ...
     Remark: Parameter -extrapath 'string' add the extra path user
     input into the end of the 'Directorys_To_Scan_List' database.
   #>
   ...
   If($extrapath -ne "false")
   {
      #Make sure User directory input exists
      If(-not(Test-Path -Path "$extrapath" -EA SilentlyContinue))
      {
         Write-Host "* ERROR: not found '$extrapath'" -ForegroundColor Red -BackgroundColor Black
         Write-Host "  => HELP: Use absoluct path declarations .." -ForegroundColor Red -BackgroundColor Black
         Start-Sleep -Seconds 2;exit #Exit @ACLMitreT1574
      }
      Else
      {
         #Inputed directory path found [ -extrapath 'string' ] ..
         #Directorys to search recursive: $Env:PROGRAMFILES, ${Env:PROGRAMFILES(x86)}, $Env:LOCALAPPDATA\Programs, + -ExtraPath 'string'
         $RawDataBaseList = Get-ChildItem  -Path "$Env:PROGRAMFILES", "${Env:PROGRAMFILES(x86)}", "$Env:LOCALAPPDATA\Programs", "$extrapath" -Recurse -ErrorAction SilentlyContinue -Force | Where-Object {
            $_.PSIsContainer -and $_.FullName -iNotMatch '(.DLL|.EXE)$' } | Select-Object -ExpandProperty FullName
      }   
   }
   Else
   {
      #Default directory scan without [ -extrapath 'string' ] declarations 
...
If($Action -ieq "reg")
{
   ...
   {
      $FinalGroupList = @(#Defaul Group Name List
   ...
   ForEach($Token in $ParseData)
   {
      ...
      ForEach($GroupServiceName in $FinalGroupList)
      {
         ...
         $CleanOutput = (Get-Acl -Path "$Token").Access | Select-Object * | Where-Object {## Search for Everyone:(F) registry service permissions (default)
            $_.IdentityReference -Match "^($GroupServiceName)" -and $_.RegistryRights -Match "^(FullControl)"
         }
         ...
         If($Verb -ieq "True")
         {
            ...
            write-host "[VERBOSE] Identity: " -ForegroundColor Blue -BackgroundColor Black -NoNewLine;
            write-host "$GroupServiceName" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
            Write-Host " - Permission " -ForegroundColor Blue -BackgroundColor Black -NoNewLine;
            Write-Host "FullControl" -ForegroundColor DarkGreen -BackgroundColor Black -NoNewLine;
            Write-Host "." -ForegroundColor Blue -BackgroundColor Black;
            Start-Sleep -Milliseconds 100
         }
   ...
   If(-not($Count -gt 0) -or $Count -ieq $null)
   {
      ...
      Write-Host "[REG] none registry services found with 'FullControl' permissions."
   }
}