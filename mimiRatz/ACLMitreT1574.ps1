...
If($Action -ieq "reg")
{
   ...
   If($Count -eq 0 -or $Count -ieq $null)
   {
      ...
      If($parseData -gt 1)
      {
         Write-Host "[REG] None registry services found with FullControl:(F)"
         Write-Host "[REG] Group: '$OneMorek'" -ForegroundColor Yellow
         Write-Host "[REG] Group: '$UtilGroup'" -ForegroundColor Yellow
         Write-Host "[REG] Group: '$UserGroup'" -ForegroundColor Yellow
      }
      Else
      {
         Write-Host "[REG] None services found with FullControl:(F)"
         Write-Host "[REG] Group Name: '$UserGroup'" -ForegroundColor Yellow
      }
      Write-Host ""
