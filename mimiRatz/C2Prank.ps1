...
If($BSODwallpaper -ieq "true")
{
   ...
   .NOTES
      ...
      Manual execute the follow cmdlines to revert desktop wallpaper to original state
      Set-Itemproperty -Path 'HKCU:\Control Panel\Desktop' -Name 'Wallpaper' -Value "$OriginalWallpaperPath" -Force
      shutdown /r /t 0   
   #>
   ...
   #Revert Wallpaper to original state instructions
   write-host "* Revert Wallpaper to original state instructions" -ForegroundColor Green
   write-host "Set-Itemproperty -Path 'HKCU:\Control Panel\Desktop' -Name 'Wallpaper' -Value `"$OriginalWallpaperPath`" -Force;shutdown /r /t 0`n"
