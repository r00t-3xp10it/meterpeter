...
[CmdletBinding(PositionalBinding=$false)] param(
   ...
   [int]$DelayTime='200'             #Delay time between each loop
)
...
If($WaveFile -ieq "Critical.wav" -or $WaveFile -iNotMatch '(.wav)$')
{
   If($WaveFile -iNotMatch '(.wav)$')
   {
      ...
      write-host "  => Using default cmdlet sound effect ..." -ForegroundColor DarkYellow
...
For($i=1; $i -lt $MaxInteractions; $i++)
{
   ...
   [int]$FinalSfx = $MaxInteractions -1 #Set last interaction
