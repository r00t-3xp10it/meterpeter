...
If($BSODwallpaper -ieq "true")
{
   ...
   .NOTES
      ...
      Manual execute the follow cmdlines to revert desktop wallpaper to original state (admin privs required)
      Remove-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\system" -Name "Wallpaper" -Force -Verbose
      shutdown /r /t 0
   #>
   ...
   If($bool)
   {
      ...
      Else
      {
         New-ItemProperty -Path "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\system" -Name "Wallpaper" -Value "$Env:TMP\bsod.png"  -PropertyType REG_SZ -Force|Out-Null
      }
   ...
   {
      write-host "x" -ForegroundColor Red -NoNewline;
      write-host " error: BSODWallpaper function requires administrator privileges .." -ForegroundColor DarkGray
   }
