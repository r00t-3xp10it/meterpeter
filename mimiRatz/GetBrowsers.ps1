...
function OPERA {
    ...
    If(Test-Path "$Env:LOCALAPPDATA\Programs\Opera???\installer_prefs.json")
    {
        ...
        echo "`n`nActive Browser Tab" >> $LogFilePath\BrowserEnum.log
        echo "------------------" >> $LogFilePath\BrowserEnum.log
        ...
        echo "---------------" >> $LogFilePath\BrowserEnum.log
        $GETbooks = Get-ChildItem -Path "$Env:LOCALAPPDATA\Programs\Opera???" -Filter Bookmarks -Recurse -EA SilentlyContinue -Force
        If($GETbooks)
        {
            echo "$GETbooks" >> $LogFilePath\BrowserEnum.log             
        }
        Else
        {
        ...
        #Get browser DLLs loaded
        echo "`nOpera DLLs loaded" >> $LogFilePath\BrowserEnum.log
        echo "-----------------" >> $LogFilePath\BrowserEnum.log
        $DllsLoaded = (Get-Process Opera|select *|Select -last 1).Modules.FileName
        If($DllsLoaded)
        {
            echo $DllsLoaded >> $LogFilePath\BrowserEnum.log             
        }
        Else
        {
            $ParsingData = "{Could not find any DLLs loaded}"
            echo "$ParsingData" >> $LogFilePath\BrowserEnum.log        
        }
