...
Under **'Linux'** or **'Windows'** distros. Under Linux users required to install **powershell** and **apache2** webserver, Under Windows its optional the install of **python3** http.server to deliver payloads under LAN. If this requirements are **NOT** met, then Client (Update-KB4524147.ps1) will be written in meterpeter working directory for manual deliver.
