...
$StartBanner = @"
 ...
 allow users to fast deliver the payload on LAN networks for tests.
     * GitHub: https://github.com/r00t-3xp10it/meterpeter *
    ...
    If($Command -ieq "NetScanner")
    {
      ...
      If($choise -ieq "PingScan" -or $choise -ieq "Ping")
      {
         ...
         If($ping_choise -ieq "AddrScan")
         {
            ...
            {
               $Outputs = "table"
               Write-Host "   => set outputs to format-table .."
            }
            Else
            {
               $Outputs = "verbose"
               Write-Host "   => set outputs to verbose mode .."
            }
