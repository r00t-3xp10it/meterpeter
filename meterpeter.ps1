...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "Registry")
    {
      ...
      write-host "`n`n   Modules   Description" -ForegroundColor green;
      write-host "   -------   -----------";
      write-host "   AMSIset   Turn On/Off AMS1 using regedit";
      ...
      If($choise -ieq "AMSIset")
      {
        write-host "`n`n   Modules   Description                     Remark" -ForegroundColor green;
        write-host "   -------   -----------                     ------";
        write-host "   Disable   Disable AMS1 (regedit)          Client:User OR ADMIN - Privs Required";
        write-host "   Enable    Enable  AMS1 (regedit)          Client:User OR ADMIN - Privs Required";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
