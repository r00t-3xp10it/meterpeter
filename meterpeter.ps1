...
#   PS cmdlet sub version: v2.10.13.2
...
"@;Write-Host $AnwsomeBanner
      ...
      If($choise -ieq "Passwords" -or $choise -ieq "pass")
      {
        ...
        write-host "   Remark: Scan takes aprox 5 minuts (%userprofile% dir)" -ForegroundColor Yellow
        write-host "   Hint: Leave input fields black to use default settings." -ForegroundColor DarkGray
        write-host "`n   Fast scan examples" -ForegroundColor Yellow
        write-host "   - Use '`$Env:USERPROFILE\Any-Dir' to reduce scan time."
        write-host "   - Use 'Stop searching after' input to reduce scan time."
        write-host "`n`n   Modules   Description                 Privileges Required" -ForegroundColor green;
        write-host "   -------   -----------                 ------------------";
        write-host "   Start     Search for creds in files   UserLand";
        write-host "   Return    Return to Server Main Menu" -ForeGroundColor yellow;
